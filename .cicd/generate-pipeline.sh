...
BUILDKITE_BUILD_AGENT_QUEUE='automation-eks-scs-builder-fleet'
BUILDKITE_TEST_AGENT_QUEUE='automation-eks-scs-tester-fleet'
...
EOF
  ...
  - label: "$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - Build"
    command:
      - "git clone \$BUILDKITE_REPO scs && cd scs && $GIT_FETCH git checkout -f \$BUILDKITE_COMMIT && git submodule update --init --recursive"
      - "cd scs && ./.cicd/build.sh"
      - "cd scs && tar -pczf build.tar.gz build && buildkite-agent artifact upload build.tar.gz"
    plugins:
      - lcscs/anka#v0.6.1:
          no-volume: true
          ...
          failover-registries:
            ...
            - 'registry_2'
          pre-commands: 
            - "git clone git@github.com:lcscs/mac-anka-fleet.git && cd mac-anka-fleet && . ./ensure-tag.bash -u 12 -r 25G -a '-n'"
      - lcscs/skip-checkout#v0.1.1:
          cd: ~
    env:
      ...
      IMAGE_TAG: $(echo "$PLATFORM_JSON" | jq -r .FILE_NAME)
      PLATFORM_TYPE: $PLATFORM_TYPE
      TAG_COMMANDS: "git clone ${BUILDKITE_PULL_REQUEST_REPO:-$BUILDKITE_REPO} scs && cd scs && $GIT_FETCH git checkout -f \$BUILDKITE_COMMIT && git submodule update --init --recursive && export IMAGE_TAG=$(echo "$PLATFORM_JSON" | jq -r .FILE_NAME) && export PLATFORM_TYPE=$PLATFORM_TYPE && . ./.cicd/platforms/$PLATFORM_TYPE/$(echo "$PLATFORM_JSON" | jq -r .FILE_NAME).sh && cd ~/scs && cd .. && rm -rf scs"
...
EOF
  ...
  - label: "$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - Unit Tests"
    command:
      - "git clone \$BUILDKITE_REPO scs && cd scs && $GIT_FETCH git checkout -f \$BUILDKITE_COMMIT && git submodule update --init --recursive"
      - "cd scs && buildkite-agent artifact download build.tar.gz . --step '$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - Build' && tar -xzf build.tar.gz"
      - "cd scs && ./.cicd/test.sh scripts/parallel-test.sh"
    plugins:
      - lcscs/anka#v0.6.1:
          no-volume: true
          inherit-environment-vars: true
...
EOF
  ...
  - label: "$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - WASM Spec Tests"
    command:
      - "git clone \$BUILDKITE_REPO scs && cd scs && $GIT_FETCH git checkout -f \$BUILDKITE_COMMIT && git submodule update --init --recursive"
      - "cd scs && buildkite-agent artifact download build.tar.gz . --step '$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - Build' && tar -xzf build.tar.gz"
      - "cd scs && ./.cicd/test.sh scripts/wasm-spec-test.sh"
    plugins:
      - lcscs/anka#v0.6.1:
          no-volume: true
          inherit-environment-vars: true
...
EOF
  ...
  - label: "$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - $TEST_NAME"
    command:
      - "git clone \$BUILDKITE_REPO scs && cd scs && $GIT_FETCH git checkout -f \$BUILDKITE_COMMIT && git submodule update --init --recursive"
      - "cd scs && buildkite-agent artifact download build.tar.gz . --step '$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - Build' && tar -xzf build.tar.gz"
      - "cd scs && ./.cicd/test.sh scripts/serial-test.sh $TEST_NAME"
    plugins:
      - lcscs/anka#v0.6.1:
          no-volume: true
          inherit-environment-vars: true
...
EOF
  ...
  - label: "$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - $TEST_NAME"
    command:
      - "git clone \$BUILDKITE_REPO scs && cd scs && $GIT_FETCH git checkout -f \$BUILDKITE_COMMIT && git submodule update --init --recursive"
      - "cd scs && buildkite-agent artifact download build.tar.gz . --step '$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - Build' ${BUILD_SOURCE} && tar -xzf build.tar.gz"
      - "cd scs && ./.cicd/test.sh scripts/long-running-test.sh $TEST_NAME"
    plugins:
      - lcscs/anka#v0.6.1:
          no-volume: true
          inherit-environment-vars: true
...
# trigger lcscs-sync-from-genesis for every build
if [[ "$BUILDKITE_PIPELINE_SLUG" == 'lcscs' && -z "${SKIP_INSTALL}${SKIP_LINUX}${SKIP_DOCKER}${SKIP_SYNC_TESTS}" ]]; then
    cat <<EOF
  - label: ":chains: Sync from Genesis Test"
    trigger: "lcscs-sync-from-genesis"
    async: false
...
# trigger lcscs-resume-from-state for every build
if [[ "$BUILDKITE_PIPELINE_SLUG" == 'lcscs' && -z "${SKIP_INSTALL}${SKIP_LINUX}${SKIP_DOCKER}${SKIP_SYNC_TESTS}" ]]; then
    cat <<EOF
  - label: ":outbox_tray: Resume from State Test"
    trigger: "lcscs-resume-from-state"
    async: false
    if: build.env("BUILDKITE_TAG") == null
    build:
      message: "Triggered by $BUILDKITE_PIPELINE_SLUG build $BUILDKITE_BUILD_NUMBER"
...
cat <<EOF
  ...
  - label: ":darwin: macOS 10.14 - Package Builder"
    command:
      - "git clone \$BUILDKITE_REPO scs && cd scs && $GIT_FETCH git checkout -f \$BUILDKITE_COMMIT"
      - "cd scs && buildkite-agent artifact download build.tar.gz . --step ':darwin: macOS 10.14 - Build' && tar -xzf build.tar.gz"
      - "cd scs && ./.cicd/package.sh"
    plugins:
      - lcscs/anka#v0.6.1:
          no-volume: true
          inherit-environment-vars: true
  ...
  - label: ":beer: Brew Updater"
    command: |
      buildkite-agent artifact download lcscs.rb . --step ':darwin: macOS 10.14 - Package Builder'
      buildkite-agent artifact upload lcscs.rb
    agents:
      queue: "automation-basic-builder-fleet"
    timeout: "${TIMEOUT:-5}"
