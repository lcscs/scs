...
[[ ! -d $GIT_ROOT/scs_multiversion_builder ]] && mkdir $GIT_ROOT/scs_multiversion_builder
# pipeline config
echo 'Reading pipeline configuration file...'
[[ -f "$RAW_PIPELINE_CONFIG" ]] && cat "$RAW_PIPELINE_CONFIG" | grep -Po '^[^"/]*("((?<=\\).|[^"])*"[^"/]*)*' | jq -c .\"scs-multiversion-tests\" > "$PIPELINE_CONFIG"
...
# multiversion
cd $GIT_ROOT/scs_multiversion_builder
echo 'Downloading other versions of nodscs...'
python2.7 $GIT_ROOT/.cicd/helpers/multi_scs_docker.py
cd $GIT_ROOT
