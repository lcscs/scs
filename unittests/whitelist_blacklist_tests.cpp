...
#include <lcscs/testing/tester_network.hpp>
...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(lcscs.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(lcscs.token), contracts::lcscs_token_wasm() );
         chain->set_abi(N(lcscs.token), contracts::lcscs_token_abi().data() );
         chain->push_action( N(lcscs.token), N(create), N(lcscs.token), mvo()
              ( "issuer", "lcscs.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(lcscs.token), N(issue), N(lcscs.token), mvo()
              ( "to", "lcscs.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(lcscs.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(lcscs.token), N(alice)};
   test.init();

   test.transfer( N(lcscs.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(lcscs.token), N(bob)};
   test.init();

   test.transfer( N(lcscs.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(lcscs.token) );
   ...
   test.chain->set_code(N(bob), contracts::lcscs_token_wasm() );
   test.chain->set_abi(N(bob), contracts::lcscs_token_abi().data() );

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), contracts::lcscs_token_wasm() );
   test.chain->set_abi(N(charlie), contracts::lcscs_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(lcscs.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_lcscs ) { try {
   whitelist_blacklist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(config::system_account_name, contracts::lcscs_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'lcscs::onerror' is on the action blacklist")
                        );
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   auto auth = authority(lcscs::testing::base_tester::get_public_key(name("alice"), "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::lcscs_code_name}, 1} );

   tester1.chain->push_action( N(lcscs), N(updateauth), N(alice), mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(lcscs::testing::base_tester::get_public_key(name("bob"), "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::lcscs_code_name}, 1} );
   auth.accounts.push_back( permission_level_weight{{N(bob), config::lcscs_code_name}, 1} );

   tester1.chain->push_action( N(lcscs), N(updateauth), N(bob), mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(lcscs::testing::base_tester::get_public_key(name("charlie"), "active"));
   auth.accounts.push_back( permission_level_weight{{N(charlie), config::lcscs_code_name}, 1} );

   tester1.chain->push_action( N(lcscs), N(updateauth), N(charlie), mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const signed_transaction&> x) {
      ...
      if( act.account == N(lcscs) && act.name == N(onblock) ) return;
