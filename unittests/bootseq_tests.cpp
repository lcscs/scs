...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(lcscs.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create lcscs.msig and lcscs.token
        create_accounts({N(lcscs.msig), N(lcscs.token), N(lcscs.ram), N(lcscs.ramfee), N(lcscs.stake), N(lcscs.vpay), N(lcscs.bpay), N(lcscs.saving) });
        // Set code for the following accounts:
        //  - lcscs (code: lcscs.bios) (already set by tester constructor)
        //  - lcscs.msig (code: lcscs.msig)
        //  - lcscs.token (code: lcscs.token)
        // set_code_abi(N(lcscs.msig), contracts::lcscs_msig_wasm(), contracts::lcscs_msig_abi().data());//, &lcscs_active_pk);
        // set_code_abi(N(lcscs.token), contracts::lcscs_token_wasm(), contracts::lcscs_token_abi().data()); //, &lcscs_active_pk);

        set_code_abi(N(lcscs.msig),
                     contracts::lcscs_msig_wasm(),
                     contracts::lcscs_msig_abi().data());//, &lcscs_active_pk);
        set_code_abi(N(lcscs.token),
                     contracts::lcscs_token_wasm(),
                     contracts::lcscs_token_abi().data()); //, &lcscs_active_pk);

        // Set privileged for lcscs.msig and lcscs.token
        set_privileged(N(lcscs.msig));
        set_privileged(N(lcscs.token));

        // Verify lcscs.msig and lcscs.token is privileged
        const auto& lcscs_msig_acc = get<account_metadata_object, by_name>(N(lcscs.msig));
        BOOST_TEST(lcscs_msig_acc.is_privileged() == true);
        const auto& lcscs_token_acc = get<account_metadata_object, by_name>(N(lcscs.token));
        BOOST_TEST(lcscs_token_acc.is_privileged() == true);


        // Create SYS tokens in lcscs.token, set its manager as lcscs
        ...
        create_currency(N(lcscs.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to lcscs.system
        issue(N(lcscs.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(lcscs.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("lcscs"));
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), lcscs_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), lcscs_assert_message_exception);
