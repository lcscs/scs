...
auto create_test_block_state( std::vector<transaction_metadata_ptr> trx_metas ) {
   ...
   block->producer = lcscs::chain::config::system_account_name;

   auto priv_key = lcscs::testing::base_tester::get_private_key( block->producer, "active" );
   auto pub_key  = lcscs::testing::base_tester::get_public_key( block->producer, "active" );
