...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for lcscs reqauth action with alice's spending key
   chain.link_authority(name("alice"), name("lcscs"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("lcscs"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with lcscs reqauth
   chain.unlink_authority(name("alice"), name("lcscs"), name("reqauth"));
   ...
   // Link authority for any lcscs action with alice's scud key
   chain.link_authority(name("alice"), name("lcscs"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("lcscs"), name("first"), name("reqauth"));
...
try {
   ...
   // Creating account with lcscs. prefix with privileged account
   chain.create_account(name("lcscs.test1"));

   // Creating account with lcscs. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("lcscs.test2"), name("joe")), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'lcscs.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("lcscs"), name("lcscs.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("lcscs"), name("lcscs.any"), name("reqauth") );

   /// this should succeed because lcscs::reqauth is linked to any permission
   chain.push_reqauth(name("alice"), { permission_level{N(alice), name("spending")} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = lcscs::chain::resource_limits::resource_usage_object;
   using by_owner = lcscs::chain::resource_limits::by_owner;
