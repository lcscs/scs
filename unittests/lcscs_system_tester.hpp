...
namespace lcscs_system {

class lcscs_system_tester : public TESTER {
public:

   lcscs_system_tester()
   : lcscs_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   lcscs_system_tester(Lambda setup) {
      setup(*this);
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(lcscs.token) );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer::create_yield_function( abi_serializer_max_time ));
      }

      create_currency( N(lcscs.token), config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("lcscs") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("lcscs")) + get_balance(name("lcscs.ramfee")) + get_balance(name("lcscs.stake")) + get_balance(name("lcscs.ram")) );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& scsin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",scsin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(lcscs.token), act, N(accounts), name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(lcscs.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(lcscs.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = lcscs::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(lcscs.token), name(symbol_code), N(stat), name(symbol_code) );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "lcscs_global_state", data, abi_serializer::create_yield_function( abi_serializer_max_time ) );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(lcscs.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( name("lcscs"), name("lcscs.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( N(lcscs.msig), contracts::lcscs_msig_wasm() );
         set_abi( N(lcscs.msig), contracts::lcscs_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(lcscs.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      //stake more than 15% of total scs supply to activate chain
      transfer( name("lcscs"), name("alice1111111"), core_from_string("650000000.0000"), name("lcscs") );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::lcscs_code_name}, 1},
...
inline uint64_t M( const string& scs_str ) {
   return core_from_string( scs_str ).get_amount();
}
