...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(lcscs.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(lcscs.token), contracts::lcscs_token_wasm() );
   c.set_abi( N(lcscs.token), contracts::lcscs_token_abi().data() );
   c.produce_blocks(10);


   auto cr = c.push_action( N(lcscs.token), N(create), N(lcscs.token), mutable_variant_object()
              ("issuer",       "lcscs" )
              ("maximum_supply", core_from_string("10000000.0000"))
      );

   cr = c.push_action( N(lcscs.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "lcscs" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
      );

   cr = c.push_action( N(lcscs.token), N(transfer), config::system_account_name, mutable_variant_object()
              ("from",     "lcscs")
              ("to",       "dan" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
