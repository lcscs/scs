...
BOOST_AUTO_TEST_CASE( activate_preactivate_feature ) try {
   ...
   BOOST_CHECK_EXCEPTION( c.set_code( config::system_account_name, contracts::lcscs_bios_wasm() ),
                          wasm_exception, fc_exception_message_is("env.is_feature_activated unresolveable")
   );

   // But the old bios contract can still be set.
   c.set_code( config::system_account_name, contracts::before_preactivate_lcscs_bios_wasm() );
   c.set_abi( config::system_account_name, contracts::before_preactivate_lcscs_bios_abi().data() );
   ...
   BOOST_CHECK_EXCEPTION( c.push_action( config::system_account_name, N(reqactivated), config::system_account_name,
                           ...
                           lcscs_assert_message_exception,
                           lcscs_assert_message_is( "protocol feature is not activated" )
   );
...
BOOST_AUTO_TEST_CASE( no_duplicate_deferred_id_test ) try {
   ...
   // Retire the delayed lcscs::reqauth transaction.
   c.produce_blocks(5);
   BOOST_REQUIRE( trace1 );
   BOOST_REQUIRE_EQUAL(0, index.size());
...
BOOST_AUTO_TEST_CASE( fix_linkauth_restriction ) { try {
   ...
   auto validate_disallow = [&] (const char *code, const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link lcscs::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };

   validate_disallow("lcscs", "linkauth");
   validate_disallow("lcscs", "unlinkauth");
   validate_disallow("lcscs", "deleteauth");
   validate_disallow("lcscs", "updateauth");
   validate_disallow("lcscs", "canceldelay");
...
BOOST_AUTO_TEST_CASE( forward_setcode_test ) { try {
   ...
   //   * lcscs::setcode to set code on the lcscs is allowed (unless the rejectall account exists)
   //   * lcscs::newaccount is allowed only if it creates the rejectall account.
   c.set_code( config::system_account_name, contracts::reject_all_wasm() );
   c.produce_block();

   // Before activation, deploying a contract should work since setcode won't be forwarded to the WASM on lcscs.
   c.set_code( tester1_account, contracts::noop_wasm() );

   // Activate FORWARD_SETCODE protocol feature and then return contract on lcscs back to what it was.
   const auto& pfm = c.control->get_protocol_feature_manager();
   ...
   // After activation, deploying a contract causes setcode to be dispatched to the WASM on lcscs,
   ...
   BOOST_REQUIRE_EXCEPTION( c.set_code( tester2_account, contracts::noop_wasm() ),
                            lcscs_assert_message_exception,
                            lcscs_assert_message_is( "rejecting all actions" ) );
   ...
   // It will now not be possible to deploy the reject_all contract to the lcscs account,
   ...
   // already deployed on lcscs.
   c2.create_account( N(rejectall) );
   c2.produce_block();
   // Now all actions dispatched to the lcscs account should be rejected.

   // However, it should still be possible to set the bios contract because the WASM on lcscs is called after the
...
BOOST_AUTO_TEST_CASE( get_sender_test ) { try {
   ...
   BOOST_CHECK_EXCEPTION(  c.push_action( tester1_account, N(sendinline), tester1_account, mutable_variant_object()
                           ...
                           lcscs_assert_message_exception,
                           lcscs_assert_message_is( "sender did not match" ) );
...
BOOST_AUTO_TEST_CASE( webauthn_producer ) { try {
   ...
   BOOST_CHECK_THROW(
      ...
      lcscs::chain::unactivated_key_type
   );
...
BOOST_AUTO_TEST_CASE( webauthn_create_account ) { try {
   ...
   BOOST_CHECK_THROW(c.push_transaction(trx), lcscs::chain::unactivated_key_type);
...
BOOST_AUTO_TEST_CASE( webauthn_recover_key ) { try {
   ...
   BOOST_CHECK_THROW(c.push_transaction(trx), lcscs::chain::unactivated_signature_type);
...
BOOST_AUTO_TEST_CASE( producer_schedule_change_extension_test ) { try {
   ...
   { // ensure producer_schedule_change_extension is rejected
      ...
      bad_block->producer_signature = remote.get_private_key(N(lcscs), "active").sign(sig_digest);
...
BOOST_AUTO_TEST_CASE( wtmsig_block_signing_inflight_legacy_test ) { try {
   ...
   c.preactivate_protocol_features( {*d} );
   vector<legacy::producer_key> sched = {{N(lcscs), c.get_public_key(N(lcscs), "bsk")}};
   ...
   c.block_signing_private_keys.emplace(get_public_key(N(lcscs), "bsk"), get_private_key(N(lcscs), "bsk"));
   c.produce_block();
