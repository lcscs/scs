...
class lcscs_token_tester : public tester {
public:

   lcscs_token_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(lcscs.token) } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = lcscs::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(lcscs.token), acc, N(accounts), name(symbol_code) );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(lcscs.token), N(create), mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(lcscs_token_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, lcscs_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, lcscs_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, lcscs_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, lcscs_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, lcscs_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, lcscs_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, lcscs_token_tester ) try {
