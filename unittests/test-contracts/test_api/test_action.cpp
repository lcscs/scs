#include <lcscslib/action.hpp>
#include <lcscslib/chain.h>
#include <lcscslib/crypto.h>
#include <lcscslib/datastream.hpp>
#include <lcscslib/db.h>
#include <lcscslib/lcscs.hpp>
#include <lcscslib/print.hpp>
#include <lcscslib/privileged.h>
#include <lcscslib/transaction.hpp>
...
void test_action::read_action_normal() {
   ...
   lcscs_assert( action_data_size() == sizeof(dummy_action), "action_size() == sizeof(dummy_action)" );

   total = read_action_data( buffer, 30 );
   lcscs_assert( total == sizeof(dummy_action) , "read_action(30)" );

   total = read_action_data( buffer, 100 );
   lcscs_assert(total == sizeof(dummy_action) , "read_action(100)" );

   total = read_action_data(buffer, 5);
   lcscs_assert( total == 5 , "read_action(5)" );

   total = read_action_data(buffer, sizeof(dummy_action) );
   lcscs_assert( total == sizeof(dummy_action), "read_action(sizeof(dummy_action))" );

   dummy_action *dummy13 = reinterpret_cast<dummy_action *>(buffer);

   lcscs_assert( dummy13->a == DUMMY_ACTION_DEFAULT_A, "dummy13->a == DUMMY_ACTION_DEFAULT_A" );
   lcscs_assert( dummy13->b == DUMMY_ACTION_DEFAULT_B, "dummy13->b == DUMMY_ACTION_DEFAULT_B" );
   lcscs_assert( dummy13->c == DUMMY_ACTION_DEFAULT_C, "dummy13->c == DUMMY_ACTION_DEFAULT_C" );
}
...
void test_action::test_dummy_action() {
   ...
   lcscs_assert( total > 0, "get_action failed" );
   lcscs::action act = lcscs::get_action( 1, 0 );
   lcscs_assert( act.authorization.back().actor == "testapi"_n, "incorrect permission actor" );
   lcscs_assert( act.authorization.back().permission == "active"_n, "incorrect permission name" );
   lcscs_assert( lcscs::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
   lcscs_assert( act.account == "testapi"_n, "expected testapi account" );
   ...
   if ( dum13.b == 200 ) {
      ...
      lcscs_assert( false, "get_context_free_data() not allowed in non-context free action" );
   } else {
      lcscs_assert( dum13.a == DUMMY_ACTION_DEFAULT_A, "dum13.a == DUMMY_ACTION_DEFAULT_A" );
      lcscs_assert( dum13.b == DUMMY_ACTION_DEFAULT_B, "dum13.b == DUMMY_ACTION_DEFAULT_B" );
      lcscs_assert( dum13.c == DUMMY_ACTION_DEFAULT_C, "dum13.c == DUMMY_ACTION_DEFAULT_C" );
   }
}
...
void test_action::test_cf_action() {
   ...
   lcscs::action act = lcscs::get_action( 0, 0 );
   ...
   if ( cfa.payload == 100 ) {
      ...
      lcscs_assert( size > 0, "size determination failed" );
      std::vector<char> cfd( static_cast<size_t>(size) );
      size = get_context_free_data( cfa.cfd_idx, &cfd[0], static_cast<size_t>(size) );
      lcscs_assert(static_cast<size_t>(size) == cfd.size(), "get_context_free_data failed" );
      uint32_t v = lcscs::unpack<uint32_t>( &cfd[0], cfd.size() );
      lcscs_assert( v == cfa.payload, "invalid value" );
      ...
      lcscs_assert( false, "db_api should not be allowed" );
      ...
      lcscs::action dum_act;
      dum_act.send();
      lcscs_assert( false, "action send should not be allowed" );
   } else if ( cfa.payload == 206 ) {
      lcscs::require_auth("test"_n);
      lcscs_assert( false, "authorization_api should not be allowed" );
   } else if ( cfa.payload == 207 ) {
      now();
      lcscs_assert( false, "system_api should not be allowed" );
      ...
      lcscs_assert( false, "transaction_api should not be allowed" );
...
void test_action::require_notice( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   if( receiver == "testapi"_n.value ) {
      lcscs::require_recipient( "acc1"_n );
      lcscs::require_recipient( "acc2"_n );
      lcscs::require_recipient( "acc1"_n, "acc2"_n );
      lcscs_assert( false, "Should've failed" );
...
void test_action::require_notice_tests( uint64_t receiver, uint64_t code, uint64_t action ) {
   lcscs::print( "require_notice_tests" );
   if( receiver == "testapi"_n.value ) {
      lcscs::print("require_recipient( \"acc5\"_n )");
      lcscs::require_recipient("acc5"_n);
   } else if( receiver == "acc5"_n.value ) {
      lcscs::print("require_recipient( \"testapi\"_n )");
      lcscs::require_recipient("testapi"_n);
   }
}

void test_action::require_auth() {
   prints("require_auth");
   lcscs::require_auth("acc3"_n);
   lcscs::require_auth("acc4"_n);
}

void test_action::assert_false() {
   lcscs_assert( false, "test_action::assert_false" );
}

void test_action::assert_true() {
   lcscs_assert( true, "test_action::assert_true" );
}
...
void test_action::test_abort() {
   abort();
   lcscs_assert( false, "should've aborted" );
}

void test_action::test_publication_time() {
   uint64_t pub_time = 0;
   uint32_t total = read_action_data( &pub_time, sizeof(uint64_t) );
   lcscs_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)" );
   lcscs_assert( pub_time == publication_time(), "pub_time == publication_time()" );
}
...
void test_action::test_current_receiver( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   lcscs_assert( receiver == cur_rec.value, "the current receiver does not match" );
}
...
void test_action::test_current_time() {
   ...
   lcscs_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)" );
   lcscs_assert( tmp == current_time(), "tmp == current_time()" );
}
...
void test_action::test_ram_billing_in_notify( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   lcscs_assert( total == sizeof(uint128_t), "total == sizeof(uint128_t)" );

   uint64_t to_notify = tmp >> 64;
   uint64_t payer = tmp & 0xFFFFFFFFFFFFFFFFULL;

   if( code == receiver ) {
      lcscs::require_recipient( name{to_notify} );
   } else {
      lcscs_assert( to_notify == receiver, "notified recipient other than the one specified in to_notify" );
...
void test_action::test_action_ordinal1(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (receiver == "testapi"_n.value) {
      print("exec 1");
      lcscs::require_recipient( "bob"_n ); //-> exec 2 which would then cause execution of 4, 10

      lcscs::action act1({name(_self), "active"_n}, name(_self),
      ...
      if (is_account("fail1"_n)) {
         lcscs_assert(false, "fail at point 1");
      }

      lcscs::action act2({name(_self), "active"_n}, name(_self),
                         name(WASM_TEST_ACTION("test_action", "test_action_ordinal3")),
                         std::tuple<>());
      act2.send(); // -> exec 9

      lcscs::require_recipient( "charlie"_n ); // -> exec 3 which would then cause execution of 11
      ...
      lcscs::require_recipient( "david"_n );  // -> exec 4
      ...
      if (is_account("fail3"_n)) {
         lcscs_assert(false, "fail at point 3");
      }

   } else if (receiver == "david"_n.value) {
      print("exec 4");
   } else {
      lcscs_assert(false, "assert failed at test_action::test_action_ordinal1");
   }
}
void test_action::test_action_ordinal2(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (receiver == "testapi"_n.value) {
      print("exec 5");
      lcscs::require_recipient( "david"_n ); // -> exec 6
      lcscs::require_recipient( "erin"_n ); // -> exec 7
      ...
      lcscs_assert(false, "assert failed at test_action::test_action_ordinal2");
   }
}
...
void test_action::test_action_ordinal3(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (is_account("failnine"_n)) {
      lcscs_assert(false, "fail at point 9");
   }
}
