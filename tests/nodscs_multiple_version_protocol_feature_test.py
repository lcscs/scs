...
###############################################################
# nodscs_multiple_version_protocol_feature_test
#
# Test for verifying that older versions of nodscs can work with newer versions of nodscs.
#
###############################################################
...
def restartNode(node: Node, nodeId, chainArg=None, addSwapFlags=None, nodscsPath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(nodeId, chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, cachePopen=True, nodscsPath=nodscsPath)
...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extraNodscsArgs=" --plugin lcscs::producer_api_plugin ",
    ...
    # Finally, when we restart the 4th node with the version of nodscs that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodscsPath="programs/nodscs/nodscs")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", nodscsPath="programs/nodscs/nodscs")
