...
###############################################################
# nodscs_producer_watermark_test
# --dump-error-details <Upon error print etc/lcscs/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
def setProds(sharedProdKey):
    ...
    trans=cluster.biosNode.pushMessage("lcscs", "setprods", setProdsStr, opts)
...
try:
    ...
    while tries > 0:
        ...
        info = node.getInfo()
        if node.infoValid and node.lastRetrievedHeadBlockProducer != "lcscs":
            break
        time.sleep(1)
        tries = tries-1
    if tries == 0:
