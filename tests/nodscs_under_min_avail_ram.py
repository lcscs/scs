...
###############################################################
# nodscs_under_min_avail_ram
#
# Sets up 4 producing nodes using --chain-state-db-guard-size-mb and --chain-state-db-size-mb to verify that nodscs will
...
try:
    ...
    maxRAMValue=1010
    extraNodscsArgs=" %s %d %s %d  --http-max-response-time-ms 990000 " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    if cluster.launch(onlyBios=False, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, extraNodscsArgs=extraNodscsArgs, useBiosBootFile=False) is False:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.lcscsAccount])
    ...
    # create accounts via lcscs as otherwise a bid is needed
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.lcscsAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.lcscsAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.lcscsAccount.name, account.name))
        nodes[0].transferFunds(cluster.lcscsAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.lcscsAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.lcscsAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.lcscsAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.lcscsAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                        Print("Failed to push create action to lcscs contract for %d consecutive times, looks like nodscs already exited." % (timeOutCount))
                        keepProcessing=False
                        break

                    Print("Failed to push create action to lcscs contract. sleep for 5 seconds")
    ...
    # min and max are subjective, just assigned to make sure that many small changes in nodscs don't 
    ...
    while keepProcessing and count < 40:
        ...
        for fromIndexOffset in range(namedAccounts.numAccounts):
            ...
            try:
                trans=nodes[count % numNodes].pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Print("Failed to push create action to lcscs contract. sleep for 60 seconds")
                    time.sleep(60)
                time.sleep(1)
            except TypeError as ex:
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killscsInstances=killscsInstances, killWallet=killWallet, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
