...
killAll()
{
  programs/lcscs-launcher/lcscs-launcher -k 15
}

cleanup()
{
    rm -rf etc/lcscs/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clscs/clscs get info)"
  verifyErrorCode "clscs get info"
...
# stand up nodscs cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodscs \"--plugin lcscs::wallet_api_plugin\" $launcherOpts
programs/lcscs-launcher/lcscs-launcher --nodscs "--plugin lcscs::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clscs/clscs --port $port2 get block 1)"
    verifyErrorCode "clscs get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clscs/clscs create key)"
verifyErrorCode "clscs create key"
...
PASSWORD_INITA="$(programs/clscs/clscs wallet create --name inita)"
verifyErrorCode "clscs wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clscs/clscs wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clscs wallet import"
programs/clscs/clscs wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clscs wallet import"
programs/clscs/clscs wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clscs wallet import"
programs/clscs/clscs wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clscs wallet import"
...
ACCOUNT_INFO="$(programs/clscs/clscs create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clscs create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clscs/clscs get account testera)"
verifyErrorCode "clscs get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clscs/clscs transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clscs transfer"
   ...
   TRANS_INFO="$(programs/clscs/clscs --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clscs get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
