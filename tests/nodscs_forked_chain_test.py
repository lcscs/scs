...
try:
    ...
    specificExtraNodscsArgs={}
    # producer nodes will be mapped to 0 through totalProducerNodes-1, so the number totalProducerNodes will be the non-producing node
    specificExtraNodscsArgs[totalProducerNodes]="--plugin lcscs::test_control_api_plugin"
    ...
    if cluster.launch(prodCount=prodCount, topo="bridge", pnodes=totalProducerNodes,
                      ...
                      useBiosBootFile=False, specificExtraNodscsArgs=specificExtraNodscsArgs) is False:
        Utils.cmdError("launcher")
        Utils.errorExit("Failed to stand up scs cluster.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.lcscsAccount,accounts[0],accounts[1],accounts[2],accounts[3],accounts[4]])
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.lcscsAccount, stakedDeposit=0, waitForTransBlock=True, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.lcscsAccount.name, account.name))
        node.transferFunds(cluster.lcscsAccount, account, transferAmount, "test transfer", waitForTransBlock=True)
