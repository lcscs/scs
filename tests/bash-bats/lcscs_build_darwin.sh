...
export SCRIPT_LOCATION="scripts/lcscs_build.sh"
export TEST_LABEL="[lcscs_build_darwin]"
...
## Needed to load lcscs_build_ files properly; it can be empty
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting lcscs Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "brew tap lcscs/lcscs") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting lcscs Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "lcscs has been successfully built") ]] || exit
}