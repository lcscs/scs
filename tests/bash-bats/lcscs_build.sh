...
SCRIPT_LOCATION="scripts/lcscs_build.sh"
TEST_LABEL="[lcscs_build]"

###################################################################
# ON MAC YOU NEED TO FULLY UNINSTALL lcscs BEFORE THESE WILL PASS #
###################################################################
...
@test "${TEST_LABEL} > Testing Arguments & Options" {
    ...
    if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
        ...
        [[ ! -z $(echo "${output}" | grep "lcscs compiler checks require the 'which'") ]] || exit
    fi
    ...
    # lack of -i
    [[ ! -z $(echo "${output}" | grep "lcscs_INSTALL_DIR: ${HOME}/lcscs/${lcscs_VERSION}") ]] || exit
    ## -o
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -o Debug -P"
    [[ ! -z $(echo "${output}" | grep "CMAKE_BUILD_TYPE: Debug") ]] || exit
    ## -s
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -s scs2 -P"
    [[ ! -z $(echo "${output}" | grep "CORE_SYMBOL_NAME: scs2") ]] || exit
    ## -b
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -b /test -P"
    [[ ! -z $(echo "${output}" | grep "BOOST_LOCATION: /test") ]] || exit
    ## -i
    run bash -c "printf \"y\ny\nn\nn\n\"| ./$SCRIPT_LOCATION -i /NEWPATH -P"
    [[ ! -z $(echo "${output}" | grep "lcscs_INSTALL_DIR: /NEWPATH") ]] || exit
    [[ ! -z $(echo "${output}" | grep "TEMP_DIR: ${HOME}/tmp") ]] || exit
        ...
        [[ ! -z $(echo "${output}" | grep "lcscs_INSTALL_DIR: $TEMP_DIR/NEWPATH") ]] || exit
