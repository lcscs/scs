...
SCRIPT_LOCATION="scripts/lcscs_uninstall.sh"
TEST_LABEL="[lcscs_uninstall]"
...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - lcscs Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled lcscs Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i lcscstest
  [[ $output =~ .*/lcscstest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/lcscs" ]] && [[ ! $output =~ ".local/share/lcscs" ]]) || exit
  [[ ! $output =~ "lcscs Removal Complete" ]] || exit
}
