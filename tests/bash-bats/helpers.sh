...
@test "${TEST_LABEL} > previous install prompt" {
  ...
  [[ -z $(echo "${output}") ]] || exit
  # Exists, prompt
  mkdir -p $lcscs_INSTALL_DIR
  run previous-install-prompt
  [[ ! -z $(echo "${output}" | grep "lcscs has already been installed into ${lcscs_INSTALL_DIR}") ]] || exit
  rm -rf $lcscs_INSTALL_DIR
}
