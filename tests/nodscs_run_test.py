...
# nodscs_run_test
#
# General test that tests a wide range of general use actions around nodscs and kscsd
#
###############################################################
...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.lcscsAccount)
    ...
    contractDir="unittests/contracts/lcscs.token"
    wasmFile="lcscs.token.wasm"
    abiFile="lcscs.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getscsAccountFromDb(currencyAccount.name)
        ...
        if abiName != "account" or abiActionName != "close" or abiType != "close":
            errorExit("FAILURE - get scs account failed", raw=True)
    ...
    account=node.getscsAccount(defproduceraAccount.name, exitOnError=True)
    ...
    if not enableMongo:
        ...
        rawAccount=node.getscsAccount(defproduceraAccount.name, exitOnError=True, returnType=ReturnType.raw)
