...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = N(lcscs.token) )
{
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   create_accounts({ N(lcscs.token), N(lcscs.ram), N(lcscs.ramfee), N(lcscs.stake),
      N(lcscs.bpay), N(lcscs.vpay), N(lcscs.saving), N(lcscs.names) });

   std::vector<account_name> accs{N(inita), N(initb), N(initc), N(initd)};
   create_accounts(accs);
   produce_block();

   set_code( N(lcscs.token), contracts::lcscs_token_wasm() );
   set_abi( N(lcscs.token), contracts::lcscs_token_abi().data() );
   produce_blocks(1);

   // create currency
   auto act = mutable_variant_object()
         ("issuer",       "lcscs")
         ("maximum_supply", lcscs::chain::asset::from_string("1000000000.0000 SYS"));
   push_action(N(lcscs.token), N(create), N(lcscs.token), act );

   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, lcscs::chain::asset::from_string("999.0000 SYS") );
   }
   produce_blocks(1);

   // iterate over scope
   lcscs::chain_apis::read_only plugin(*(this->control), {}, fc::microseconds::maximum());
   lcscs::chain_apis::read_only::get_table_by_scope_params param{N(lcscs.token), N(accounts), "inita", "", 10};
   lcscs::chain_apis::read_only::get_table_by_scope_result result = plugin.read_only::get_table_by_scope(param);

   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   BOOST_REQUIRE_EQUAL("", result.more);
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name(N(lcscs.token)), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name(N(inita)), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name(N(accounts)), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name(N(lcscs)), result.rows[0].payer);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, lcscs::chain::asset::from_string("10000.0000 SYS") );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "lcscs")
         ("maximum_supply", lcscs::chain::asset::from_string("1000000000.0000 AAA"));
   push_action(N(lcscs.token), N(create), N(lcscs.token), act );
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, lcscs::chain::asset::from_string("9999.0000 AAA") );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "lcscs")
         ("maximum_supply", lcscs::chain::asset::from_string("1000000000.0000 CCC"));
   push_action(N(lcscs.token), N(create), N(lcscs.token), act );
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, lcscs::chain::asset::from_string("7777.0000 CCC") );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "lcscs")
         ("maximum_supply", lcscs::chain::asset::from_string("1000000000.0000 BBB"));
   push_action(N(lcscs.token), N(create), N(lcscs.token), act );
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, lcscs::chain::asset::from_string("8888.0000 BBB") );
   }
   produce_blocks(1);

   // get table: normal case
   lcscs::chain_apis::read_only plugin(*(this->control), {}, fc::microseconds::maximum());
   lcscs::chain_apis::read_only::get_table_rows_params p;
   p.code = N(lcscs.token);
   p.scope = "inita";
   p.table = N(accounts);
   p.json = true;
   p.index_position = "primary";
   lcscs::chain_apis::read_only::get_table_rows_result result = plugin.read_only::get_table_rows(p);
   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("lcscs", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("lcscs", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("lcscs", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("lcscs", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   set_code( config::system_account_name, contracts::lcscs_system_wasm() );
   set_abi( config::system_account_name, contracts::lcscs_system_abi().data() );
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( N(lcscs), N(bidname), bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
   ...
   bidname(N(inita), N(com), lcscs::chain::asset::from_string("10.0000 SYS"));
   bidname(N(initb), N(org), lcscs::chain::asset::from_string("11.0000 SYS"));
   bidname(N(initc), N(io), lcscs::chain::asset::from_string("12.0000 SYS"));
   bidname(N(initd), N(html), lcscs::chain::asset::from_string("14.0000 SYS"));
   produce_blocks(1);
