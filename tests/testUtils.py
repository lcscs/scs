...
class Utils:
    ...
    scsClientPath="programs/clscs/clscs"
    MiscscsClientArgs="--no-auto-kscsd"

    scsWalletName="kscsd"
    scsWalletPath="programs/kscsd/"+ scsWalletName

    scsServerName="nodscs"
    scsServerPath="programs/nodscs/"+ scsServerName

    scsLauncherPath="programs/lcscs-launcher/lcscs-launcher"
    MongoPath="mongo"
    ShuttingDown=False
    CheckOutputDeque=deque(maxlen=10)

    scsBlockLogPath="programs/lcscs-blocklog/lcscs-blocklog"
    ...
    def getBlockLog(blockLogLocation, blockLogAction=BlockLogAction.return_blocks, outputFile=None, first=None, last=None, throwException=False, silentErrors=False, exitOnError=False):
        ...
        cmd="%s --blocks-dir %s --as-json-array %s%s%s%s" % (Utils.scsBlockLogPath, blockLogLocation, outputFileStr, firstStr, lastStr, blockLogActionStr)
