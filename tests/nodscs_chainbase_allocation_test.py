...
###############################################################
# nodscs_chainbase_allocation_test
#
...
try:
    ...
    # (automatically taken care by the automatically generated lcscs account)
    assert cluster.launch(
        ...
        specificExtraNodscsArgs={
            1:"--read-mode irreversible --plugin lcscs::producer_api_plugin"})
    ...
    cmd = "create account -j lcscs sample scs6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\
         scs6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV --delay-sec 600 -p lcscs"
    trans = producerNode.processClscsCmd(cmd, cmd, silentErrors=False)
    assert trans

    # Schedule a new producer to trigger new producer schedule for "global_property_object"
    newProducerAcc = Account("newprod")
    newProducerAcc.ownerPublicKey = "scs6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
    newProducerAcc.activePublicKey = "scs6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
    producerNode.createAccount(newProducerAcc, cluster.lcscsAccount)

    setProdsStr = '{"schedule": ['
    setProdsStr += '{"producer_name":' + newProducerAcc.name + ',"block_signing_key":' + newProducerAcc.activePublicKey + '}'
    setProdsStr += ']}'
    cmd="push action -j lcscs setprods '{}' -p lcscs".format(setProdsStr)
    trans = producerNode.processClscsCmd(cmd, cmd, silentErrors=False)
    assert trans
