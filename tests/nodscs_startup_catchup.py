...
try:
    ...
    for nodeNum in range(txnGenNodeNum, txnGenNodeNum+startedNonProdNodes):
        specificExtraNodscsArgs[nodeNum]="--plugin lcscs::txn_test_gen_plugin --txn-test-gen-account-prefix txntestacct"
    ...
    if cluster.launch(prodCount=prodCount, onlyBios=False, pnodes=pnodes, totalNodes=totalNodes, totalProducers=pnodes*prodCount,
                      useBiosBootFile=False, specificExtraNodscsArgs=specificExtraNodscsArgs, unstartedNodes=catchupCount, loadSystemContract=False) is False:
    ...
    txnGenNodes[0].txnGenCreateTestAccounts(cluster.lcscsAccount.name, cluster.lcscsAccount.activePrivateKey)
