...
try:
    ...
    if not dontLaunch:
        ...
        killscsInstances=False

        print("Stand up walletd")
        if walletMgr.launch() is False:
            cmdError("%s" % (WalletdName))
            errorExit("Failed to stand up scs walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.lcscsAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.lcscsAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.lcscsAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.lcscsAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.lcscsAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.lcscsAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.lcscsAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountscsBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountscsBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killscsInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
