...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="lcscs-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kscsd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="scstest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kscsd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: scs server host
        port: scs server port
        walletHost: scs wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures scs mongo plugin
        ...
        self.lcscsAccount=self.defProducerAccounts["lcscs"]= Account("lcscs")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodscsArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraNodscsArgs=None, onlySetProds=False,
        ...
        extraNodscsArgs: string of arguments to pass through to each nodoes instance (via --nodscs flag on launcher)
        ...
        specificExtraNodscsArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodscs flags on launcher), example: { "5" : "--plugin lcscs::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the lcscs.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.scsServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodscs default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.scsLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodscsArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodscsArgs += " --plugin lcscs::wallet_api_plugin"
        if self.enableMongo:
            nodscsArgs += " --plugin lcscs::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodscsArgs is not None:
            assert(isinstance(extraNodscsArgs, str))
            nodscsArgs += extraNodscsArgs
        if Utils.Debug:
            nodscsArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodscsArgs += " --plugin lcscs::producer_api_plugin"

        if nodscsArgs:
            cmdArr.append("--nodscs")
            cmdArr.append(nodscsArgs)

        if specificExtraNodscsArgs is not None:
            assert(isinstance(specificExtraNodscsArgs, dict))
            for nodeNum,arg in specificExtraNodscsArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodscs")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodscs")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.scsServerName, startedNodes, len(nodes)))
            return False
        ...
        self.lcscsAccount=self.defProducerAccounts["lcscs"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.scsClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.scsClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on scs server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextscsIdx: %d, n: %d" % (nextscsIdx, n))
                nextscsIdx=(nextscsIdx + 1)%count
                if not self.nodes[nextscsIdx].killed:
                    #Utils.Print("nextscsIdx: %d" % (nextscsIdx))
            ...
            #Utils.Print("nextscsIdx: %d, count: %d" % (nextscsIdx, count))
            node=self.nodes[nextscsIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.scsServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on scs node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getscsBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.lcscsAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse lcscs private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by lcscs-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/lcscs.bios",
            "BIOS_CURRENCY_SYMBOL": CORE_SYMBOL,
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/lcscs.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down scs cluster.")
            return None
        ...
        lcscsName="lcscs"
        lcscsKeys=producerKeys[lcscsName]
        lcscsAccount=Account(lcscsName)
        lcscsAccount.ownerPrivateKey=lcscsKeys["private"]
        lcscsAccount.ownerPublicKey=lcscsKeys["public"]
        lcscsAccount.activePrivateKey=lcscsKeys["private"]
        lcscsAccount.activePublicKey=lcscsKeys["public"]
        producerKeys.pop(lcscsName)

        if not self.walletMgr.importKey(lcscsAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (lcscsName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="lcscs.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"lcscs\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init lcscs transfer")
            opts="--permission lcscs@active"
            if name != "lcscs":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from lcscs.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(lcscsAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, lcscsAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission lcscs@active"
                    myTrans=biosNode.pushMessage("lcscs", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("lcscs", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but lcscs).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "lcscs"
        ...
        lcscsTokenAccount=copy.deepcopy(lcscsAccount)
        lcscsTokenAccount.name="lcscs.token"
        trans=biosNode.createAccount(lcscsTokenAccount, lcscsAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (lcscsTokenAccount.name))
            return None

        lcscsRamAccount=copy.deepcopy(lcscsAccount)
        lcscsRamAccount.name="lcscs.ram"
        trans=biosNode.createAccount(lcscsRamAccount, lcscsAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (lcscsRamAccount.name))
            return None

        lcscsRamfeeAccount=copy.deepcopy(lcscsAccount)
        lcscsRamfeeAccount.name="lcscs.ramfee"
        trans=biosNode.createAccount(lcscsRamfeeAccount, lcscsAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (lcscsRamfeeAccount.name))
            return None

        lcscsStakeAccount=copy.deepcopy(lcscsAccount)
        lcscsStakeAccount.name="lcscs.stake"
        trans=biosNode.createAccount(lcscsStakeAccount, lcscsAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (lcscsStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(lcscsTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=lcscsTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (lcscsAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to lcscs contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (lcscsAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (lcscsAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to lcscs contract.")
            return None
        ...
        Utils.Print("Verify lcscs issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountscsBalanceStr(lcscsAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (lcscsAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (lcscsAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (lcscsTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (lcscsAccount.name)
            trans=biosNode.pushMessage(lcscsAccount.name, action, data, opts)
    ...
    @staticmethod
    def pgrepscsServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.scsServerName)
    ...
    @staticmethod
    def pgrepscsServerPattern(nodeInstance):
        dataLocation=Utils.getNodeDataDir(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of scsInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepscsServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepscsServerPattern(nodeNum)
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.scsServerName, pattern))
            return None
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepscsServers(timeout=timeout)
        pattern=Cluster.pgrepscsServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.scsServerName, pattern))
    ...
    # Kills a percentange of scs instances starting from the tail and update scsInstanceInfos state
    def killSomescsInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.scsServerName, killSignal))
    ...
    def relaunchscsInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodscs instances. allInstances will kill all nodscs instances running on the system."""
        cmd="%s -k 9" % (Utils.scsLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down scs cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the scs server
            cmd="pkill -9 %s" % (Utils.scsServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down scs cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodscs instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodscs instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
