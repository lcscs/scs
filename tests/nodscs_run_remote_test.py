...
###############################################################
# nodscs_run_remote_test
#
#  Tests remote capability of the nodscs_run_test. Test will setup cluster and pass nodes info to nodscs_run_test. E.g.
#  nodscs_run_remote_test.py -v --clean-run --dump-error-detail
#
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodscs_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodscs", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killscsInstances, False, False, killAll, dumpErrorDetails)
