...
# lcscs should have the same key as defproducera
lcscs = copy.copy(defproduceraAccount)
lcscs.name = "lcscs"

Print("Info of each node:")
for i in range(len(hosts)):
    node = node0
    cmd="%s %s get info" % (testUtils.Utils.scsClientPath, node.endpointArgs)
    trans = node.runCmdReturnJson(cmd)
    Print("host %s: %s" % (hosts[i], trans))


wasmFile="lcscs.system.wasm"
abiFile="lcscs.system.abi"
Print("\nPush system contract %s %s" % (wasmFile, abiFile))
trans=node0.publishContract(lcscs.name, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish lcscs.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, lcscs)
