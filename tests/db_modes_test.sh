...
lcscs_STUFF_DIR=$(mktemp -d)
trap "rm -rf $lcscs_STUFF_DIR" EXIT
NODscs_LAUNCH_PARAMS="./programs/nodscs/nodscs -d $lcscs_STUFF_DIR --config-dir $lcscs_STUFF_DIR \
--chain-state-db-size-mb 8 --chain-state-db-guard-size-mb 0 --reversible-blocks-db-size-mb 1 \
--reversible-blocks-db-guard-size-mb 0 -e -plcscs"

run_nodscs() {
   if (( $VERBOSE == 0 )); then
      $NODscs_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" 2>/dev/null &
   else
      $NODscs_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" &
   fi
}
...
run_expect_failure() {
   ...
   trap "kill $NODscs_PID; wait $NODscs_PID; exit 1" ALRM
   sleep 10
   if wait $NODscs_PID; then exit 1; fi
   kill $TIMER_PID
   trap ALRM
}
