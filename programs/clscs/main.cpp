/**
  @defgroup scsclienttool

  @section intro Introduction to clscs

  `clscs` is a command line tool that interfaces with the REST api exposed by @ref nodscs. In order to use `clscs` you will need to
  have a local copy of `nodscs` running and configured to load the 'lcscs::chain_api_plugin'.

   clscs contains documentation for all of its commands. For a list of all commands known to clscs, simply run it with no arguments:
```
$ ./clscs
Command Line Interface to lcscs Client
Usage: programs/clscs/clscs [OPTIONS] SUBCOMMAND
...
#include <lcscs/chain/name.hpp>
#include <lcscs/chain/config.hpp>
#include <lcscs/chain/wast_to_wasm.hpp>
#include <lcscs/chain/trace.hpp>
#include <lcscs/chain_plugin/chain_plugin.hpp>
#include <lcscs/chain/contract_types.hpp>

#include <lcscs/version/version.hpp>
...
using namespace lcscs::client::help;
using namespace lcscs::client::http;
using namespace lcscs::client::localize;
using namespace lcscs::client::config;
using namespace boost::filesystem;
using auth_type = fc::static_variant<public_key_type, permission_level>;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define scsC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
//copy pasta from kscsd's main.cpp
...
string default_wallet_url = "unix://" + (determine_home_directory() / "lcscs-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
lcscs::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      auto sp = std::make_unique<lcscs::client::http::connection_param>(context, parse_url(url) + path, no_verify ? false : true, headers);
      return lcscs::client::http::do_http_call(*sp, fc::variant(v), print_request, print_response );
   }
...
lcscs::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<lcscs::chain_apis::read_only::get_info_results>();
}
...
fc::variant push_transaction( signed_transaction& trx, packed_transaction::compression_type compression = packed_transaction::compression_type::none ) {
   ...
   if (trx.signatures.size() == 0) { // #5445 can't change txn content if already signed
      ...
      } scs_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
auto abi_serializer_resolver = [](const name& account) -> fc::optional<abi_serializer> {
   ...
   if ( it == abi_cache.end() ) {
      ...
      auto abi_results = result.as<lcscs::chain_apis::read_only::get_abi_results>();
...
fc::variant json_from_file_or_string(const string& file_or_str, fc::json::parse_type ptype = fc::json::parse_type::legacy_parser)
{
   ...
   if ( !regex_search(file_or_str, r) && fc::is_regular_file(file_or_str) ) {
      ...
      } scs_RETHROW_EXCEPTIONS(json_parse_exception, "Fail to parse JSON from file: ${file}", ("file", file_or_str));

   } else {
      try {
         return fc::json::from_string(file_or_str, ptype);
      } scs_RETHROW_EXCEPTIONS(json_parse_exception, "Fail to parse JSON from string: ${string}", ("string", file_or_str));
   }
}
...
void send_actions(std::vector<chain::action>&& actions, packed_transaction::compression_type compression = packed_transaction::compression_type::none ) {
   ...
   if (tx_json_save_file.length()) {
      ...
      scsC_ASSERT(!out.fail(), "ERROR: Failed to create file \"${p}\"", ("p", tx_json_save_file));
   }
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } scs_RETHROW_EXCEPTIONS(authority_type_exception, "Invalid authority format '${data}'",
                            ("data", fc::json::to_string(authority_var, fc::time_point::maximum())))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "SCS") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } scs_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      scs_ASSERT( lcscs::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( account_name code, const string& s ) {
   static map< pair<account_name, lcscs::chain::symbol_code>, lcscs::chain::symbol> cache;
   auto a = asset::from_string( s );
   lcscs::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<lcscs::chain_apis::read_only::get_currency_stats_result>();
         auto p = cache.emplace( make_pair( code, sym ), result.max_supply.get_symbol() );
         it = p.first;
      } else {
         scs_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}", ("s", sym_str)("c", code));
      }
   }
   ...
   if ( a.decimals() < expected_symbol.decimals() ) {
      ...
      scs_THROW(symbol_type_exception, "Too many decimal digits in ${a}, only ${d} supported", ("a", a)("d", expected_symbol.decimals()));
   } // else precision matches
   return a;
}

inline asset to_asset( const string& s ) {
   return to_asset( N(scs.token), s );
}
...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->add_flag("--add-code", add_code, localized("[code] add '${code}' permission to specified permission authority", ("code", name(config::lcscs_code_name))));
      permissions->add_flag("--remove-code", remove_code, localized("[code] remove '${code}' permission from specified permission authority", ("code", name(config::lcscs_code_name))));

      add_standard_transaction_options(permissions, "account@active");

      permissions->callback([this] {
         scsC_ASSERT( !(add_code && remove_code), "ERROR: Either --add-code or --remove-code can be set" );
         scsC_ASSERT( (add_code ^ remove_code) || !authority_json_or_file.empty(), "ERROR: authority should be specified unless add or remove code permission" );
         ...
         if ( need_parent || need_auth ) {
            ...
            auto res = json.as<lcscs::chain_apis::read_only::get_account_results>();
            ...
            if ( need_auth ) {
               ...
               auto code_name = config::lcscs_code_name;
...
void ensure_kscsd_running(CLI::App* app) {
    if (no_auto_kscsd)
        return;
    // get, version, net, convert do not require kscsd
    ...
    // This extra check is necessary when running clscs like this: ./clscs ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if clscs and kscsd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kscsd").append(key_store_executable_name);
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child kscs(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (kscs.running()) {
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->callback([this] {
         ...
         } scs_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
...
struct create_account_subcommand {
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         ...
         createAccount->add_option("--buy-ram", buy_ram_scs,
      ...
      createAccount->callback([this] {
            ...
            if( owner_key_str.find('@') != string::npos ) {
               ...
               } scs_RETHROW_EXCEPTIONS( explained_exception, "Invalid owner permission level: ${permission}", ("permission", owner_key_str) )
            } else {
               try {
                  owner = public_key_type(owner_key_str);
               } scs_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str) );
            }
            ...
            if( active_key_str.empty() ) {
               ...
               } scs_RETHROW_EXCEPTIONS( explained_exception, "Invalid active permission level: ${permission}", ("permission", active_key_str) )
            } else {
               try {
                  active = public_key_type(active_key_str);
               } scs_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str) );
            }

            auto create = create_newaccount(name(creator), name(account_name), owner, active);
            if (!simple) {
               scsC_ASSERT( buy_ram_scs.size() || buy_ram_bytes_in_kbytes || buy_ram_bytes, "ERROR: One of --buy-ram, --buy-ram-kbytes or --buy-ram-bytes should have non-zero value" );
               scsC_ASSERT( !buy_ram_bytes_in_kbytes || !buy_ram_bytes, "ERROR: --buy-ram-kbytes and --buy-ram-bytes cannot be set at the same time" );
               action buyram = !buy_ram_scs.empty() ? create_buyram(name(creator), name(account_name), to_asset(buy_ram_scs))
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so clscs can still work with old buggy nodscs versions
                               // Change to voter.value when clscs no longer needs to support nodscs versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<lcscs::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when clscs no longer needs to support nodscs versions older than 1.5.0
            ...
            scs_ASSERT( 1 == res.rows.size(), multiple_voter_info, "More than one voter_info for account" );
            auto prod_vars = res.rows[0]["producers"].get_array();
            vector<lcscs::name> prods;
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         ...
         auto result = rawResult.as<lcscs::chain_apis::read_only::get_producers_result>();
...
struct get_transaction_id_subcommand {
   ...
   get_transaction_id_subcommand(CLI::App* actionRoot) {
      ...
      get_transaction_id->callback([&] {
         ...
         } scs_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_check))
      });
   }
};
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->callback([this] {
         ...
         scsC_ASSERT( !(buy_ram_amount.size()) || !buy_ram_bytes, "ERROR: --buyram and --buy-ram-bytes cannot be set at the same time" );
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "lcscs")("scope", "lcscs")("table", "namebids")
                               ("lower_bound", name(newname).to_uint64_t())
                               ("upper_bound", name(newname).to_uint64_t() + 1)
                               // Less than ideal upper_bound usage preserved so clscs can still work with old buggy nodscs versions
                               // Change to newname.value when clscs no longer needs to support nodscs versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         }
         auto result = rawResult.as<lcscs::chain_apis::read_only::get_table_rows_result>();
         // Condition in if statement below can simply be res.rows.empty() when clscs no longer needs to support nodscs versions older than 1.5.0
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->callback([this] {
         scsC_ASSERT( !kbytes || !bytes, "ERROR: --kbytes and --bytes cannot be set at the same time" );
         if (kbytes || bytes) {
...
void get_account( const string& accountName, const string& coresym, bool json_format ) {
   ...
   if (!json_format) {
      ...
      unordered_map<name, lcscs::chain_apis::permission> cache;
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of nodscs responding to the request is different than core symbol built into clscs
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'lcscs::refund' action)\n";
            } else {
...
int main( int argc, char** argv ) {
   ...
   context = lcscs::client::http::create_http_context();
   wallet_url = default_wallet_url;

   CLI::App app{"Command Line Interface to lcscs Client"};
   ...
   app.add_flag( "--no-auto-" + string(key_store_executable_name), no_auto_kscsd, localized("don't automatically launch a ${k} if one is not currently running", ("k", key_store_executable_name)));
   app.parse_complete_callback([&app]{ ensure_kscsd_running(&app);});
   ...
   pack_transaction->callback([&] {
      ...
      if( pack_action_data_flag ) {
         ...
         } scs_RETHROW_EXCEPTIONS( transaction_type_exception, "Invalid transaction format: '${data}'",
         ...
         } scs_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to convert transaction, --pack-action-data likely needed" )
      }
   });
   ...
   unpack_transaction->callback([&] {
      ...
      } scs_RETHROW_EXCEPTIONS( transaction_type_exception, "Invalid packed transaction format: '${data}'",
   ...
   pack_action_data->callback([&] {
      ...
      } scs_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse unpacked action data JSON")
   ...
   unpack_action_data->callback([&] {
      scs_ASSERT( packed_action_data_string.size() >= 2, transaction_type_exception, "No packed_action_data found" );
   ...
   getCode->callback([&] {
      ...
      try {
         ...
         //see if this is an old nodscs that doesn't support get_raw_code_and_abi
   ...
   getAccounts->callback([&] {
      ...
      } scs_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        scs_ASSERT( !wasm.empty(), wasm_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto set_abi_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        scs_ASSERT( fc::exists( abiPath ), abi_file_not_found, "no abi file found ${f}", ("f", abiPath)  );
      ...
      if (!duplicate) {
         ...
         } scs_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
         if ( shouldSend ) {
   ...
   contractSubcommand->callback([&] {
      if(!contract_clear) scs_ASSERT( !contractPath.empty(), contract_exception, " contract-dir is null ", ("f", contractPath) );
      shouldSend = false;
      set_code_callback();
      set_abi_callback();
      if (actions.size()) {
   ...
   createWallet->callback([&wallet_name, &password_file, &print_console] {
      scsC_ASSERT( !password_file.empty() ^ print_console, "ERROR: Either indicate a file using \"--file\" or pass \"--to-console\"" );
      scsC_ASSERT( password_file.empty() || !std::ofstream(password_file.c_str()).fail(), "ERROR: Failed to create file in specified path" );
   ...
   importWallet->callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         scs_THROW(private_key_type_exception, "Invalid private key")
      }
   ...
   removeKeyWallet->callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
      ...
      try {
         ...
         scs_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
      }
   ...
   auto stopKscsd = wallet->add_subcommand("stop", localized("Stop ${k}.", ("k", key_store_executable_name)));
   stopKscsd->callback([] {
      const auto& v = call(wallet_url, kscsd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success kscsd responds with empty object
   ...
   sign->callback([&] {
      ...
      scsC_ASSERT( str_private_key.empty() || str_public_key.empty(), "ERROR: Either -k/--private-key or --public-key or none of them can be set" );
      ...
      if( str_public_key.size() > 0 ) {
         ...
         } scs_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", str_public_key))
         ...
         } scs_RETHROW_EXCEPTIONS(private_key_type_exception, "Invalid private key")
   ...
   propose_action->callback([&] {
      ...
      } scs_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } scs_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            scs_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, N(scs.msig), N(propose), variant_to_bin( N(scs.msig), N(propose), args ) }});
   });
   ...
   review->callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so clscs can still work with old buggy nodscs versions
                                 // Change to name(proposal_name).value when clscs no longer needs to support nodscs versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when clscs no longer needs to support nodscs versions older than 1.5.0
      ...
      std::map<lcscs::account_name, std::pair<fc::time_point, vector<decltype(all_approvals)::iterator>>>  provided_approvers;
      ...
      if( show_approvals_in_multisig_review ) {
         ...
         if( new_multisig ) {
            for( auto& a : provided_approvers ) {
               ...
               if( rows4.empty() || rows4[0].get_object()["account"].as<lcscs::name>() != a.first ) {
                  continue;
               }
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(scs.msig), name(action), variant_to_bin( N(scs.msig), name(action), args ) }});
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(scs.msig), N(invalidate), variant_to_bin( N(scs.msig), N(invalidate), args ) }});
   });
   ...
   cancel->callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            scs_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      send_actions({chain::action{accountPermissions, N(scs.msig), N(cancel), variant_to_bin( N(scs.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            scs_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, N(scs.msig), N(exec), variant_to_bin( N(scs.msig), N(exec), args ) }});
      }
   );
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send scs.system contract action to the blockchain."));
