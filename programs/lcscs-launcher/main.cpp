...
#include <net/if.h>
#include <lcscs/chain/genesis_state.hpp>
...
using namespace lcscs::launcher::config;
...
void
scsd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string lcscs_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), lcscs_home(), instances(1) {}
};
...
struct launcher_def {
   ...
   string scsd_extra_args;
   std::map<uint,string> specific_nodscs_args;
   std::map<uint,string> specific_nodscs_installation_paths;
   testnet_def network;
   string gelf_endpoint;
   ...
   lcscs::chain::genesis_state genesis_from_file;

   void assign_name (scsd_def &node, bool is_bios);
   ...
   void prep_remote_config_dir (scsd_def &node, host_def *host);
   void launch (scsd_def &node, string &gts);
   void kill (launch_modes mode, string sig_opt);
   static string get_node_num(uint16_t node_num);
   pair<host_def, scsd_def> find_node(uint16_t node_num);
   vector<pair<host_def, scsd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    (node_executable_name, bpo::value<string>(&scsd_extra_args), ("forward " + string(node_executable_name) + " command line argument(s) to each instance of " + string(node_executable_name) + ", enclose arg(s) in quotes").c_str())
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  retrieve_paired_array_parameters(vmap, "specific-num", "specific-" + string(node_executable_name), specific_nodscs_args);
  retrieve_paired_array_parameters(vmap, "spcfc-inst-num", "spcfc-inst-" + string(node_executable_name), specific_nodscs_installation_paths);
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &scsd : binding.instances) {
          scsd.host = binding.host_name;
          scsd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(scsd.p2p_port);

          aliases.push_back (scsd.name);
        }
      }
  ...
  if (vmap.count("specific-num")) {
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodscs_args[num] = specific_args[i];
    }
  }

  char* erd_env_var = getenv ("lcscs_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable lcscs_HOME is set to the build path." << endl;
    exit (-1);
  }
...
void
launcher_def::assign_name (scsd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.lcscs_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      scsd_def scsd;
      assign_name(scsd, i == 0);
      aliases.push_back(scsd.name);
      scsd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(scsd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->lcscs_home =
          (local_id.contains (lhost->host_name) || servers.default_lcscs_home.empty()) ?
          erd : servers.default_lcscs_home;
        host_ndx++;
      } // ph_count == 0

      scsd_def scsd;
      assign_name(scsd, do_bios);
      scsd.has_db = false;
      ...
      aliases.push_back(scsd.name);
      scsd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(scsd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  scsd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->lcscs_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->lcscs_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->lcscs_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->lcscs_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->lcscs_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = lcscs::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = lcscs::mongo_db_plugin\n";
  }
  cfg << "plugin = lcscs::net_plugin\n";
  cfg << "plugin = lcscs::chain_api_plugin\n"
...
void
launcher_def::prep_remote_config_dir (scsd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->lcscs_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->lcscs_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->lcscs_home;

  cmd = "cd " + host->lcscs_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->lcscs_home
...
void
launcher_def::launch (scsd_def &instance, string &gts) {
  ...
  string install_path;
  if (instance.name != "bios" && !specific_nodscs_installation_paths.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodscs_installation_paths.count(node_num)) {
        install_path = specific_nodscs_installation_paths[node_num] + "/";
     }
  }
  string scsdcmd = install_path + "programs/nodscs/" + string(node_executable_name) + " ";
  if (skip_transaction_signatures) {
    scsdcmd += "--skip-transaction-signatures ";
  }
  if (!scsd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +lcscs::mongo_db_plugin");
       string args = std::regex_replace (scsd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       scsdcmd += args + " ";
    }
    else {
       scsdcmd += scsd_extra_args + " ";
    }
  }
  if (instance.name != "bios" && !specific_nodscs_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodscs_args.count(node_num)) {
        scsdcmd += specific_nodscs_args[node_num] + " ";
     }
  }

  if( add_enable_stale_production ) {
    scsdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  scsdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  scsdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    scsdcmd += " --genesis-timestamp " + gts;
  }
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->lcscs_home + "; nohup " + scsdcmd + " > "
    ...
    string cmd = "cd " + host->lcscs_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else if (!instance.node->dont_start) {
    cerr << "spawning child, " << scsdcmd << endl;

    bp::child c(scsdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << scsdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
    ...
    cerr << "not spawning child, " << scsdcmd << endl;
...
#if 0
void
launcher_def::kill_instance(scsd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, scsd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, scsd_def>> node_list;
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.lcscs_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const scsd_def& node = node_pair.second;
      const string node_num = node.get_node_num();
      cout << "Bouncing " << node.name << endl;
      string cmd = "./scripts/lcscs-tn_bounce.sh " + scsd_extra_args;
      if (node_num != "bios" && !specific_nodscs_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodscs_args.count(node_num_i)) {
            cmd += " " + specific_nodscs_args[node_num_i];
         }
      }

      do_command(host, node.name, { { "lcscs_HOME", host.lcscs_home }, { "lcscs_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/lcscs-tn_down.sh ";
      do_command(host, node.name,
                 { { "lcscs_HOME", host.lcscs_home }, { "lcscs_NODE", node_num }, { "lcscs_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/lcscs-tn_roll.sh ";
      do_command(*host, host_name, { { "lcscs_HOME", host->lcscs_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the lcscs-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the lcscs-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the lcscs-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << lcscs::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(lcscs_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (lcscs_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_lcscs_home) (ssh) )
