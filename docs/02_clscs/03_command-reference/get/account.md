...
### Get formatted data for user **lcscs**

```sh
clscs get account lcscs
```
```console
privileged: true
permissions: 
...
producers:     <not voted>
```
### Get JSON data for user **lcscs**

```sh
clscs get account lcscs --json
```
```json
{
  "account_name": "lcscs",
  ...
  "permissions": [{
      ...
      "required_auth": {
        "threshold": 1,
        "keys": [{
            "key": "scs6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
            "weight": 1
          }
        ],
        "accounts": [],
