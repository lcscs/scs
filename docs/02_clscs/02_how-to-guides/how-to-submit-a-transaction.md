...
[[info]]
| Be aware that if a clear text `data` field is used, clscs need to fetch copies of required ABIs using `nodscs` API. That operation has a performance overhead on `nodscs`
...
```sh
clscs push transaction TRX_FILE.json
```

* Submit a transaction from a JSON:

```sh
clscs push transaction JSON
```
