...
The `chain_plugin` is a core plugin required to process and aggregate chain data on an lcscs node.
...
```console
Config Options for lcscs::chain_plugin:
  ...
  --wasm-runtime scs-vm|scs-vm-jit      Override default WASM runtime (wabt)
  --scs-vm-oc-enable                    Enable optimized compilation in WASM
