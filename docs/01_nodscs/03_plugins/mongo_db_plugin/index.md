...
The optional `lcscs::mongo_db_plugin` provides archiving of blockchain data into a MongoDB. It is recommended that the plugin be added to a non-producing node as it is designed to shut down on any failed insert into the MongoDB and it is resource intensive. For best results dedicate a `nodscs` instance to running this one plugin. The rationale behind this shutdown on error is so that any issues with connectivity or the mongo database can be fixed and `nodscs` can be restarted without having to resync or replay.
...
* Action data is stored on chain as raw bytes. This plugin attempts to use associated ABI on accounts to deserialize the raw bytes into expanded `abi_def` form for storage into mongo. Note that invalid or missing ABI on a contract will result in the action data being stored as raw bytes. For example the lcscs system contract does not provide ABI for the `onblock` action so it is stored as raw bytes.
...
* It is recommended that a large `--abi-serializer-max-time-ms` value be passed into the `nodscs` running the `mongo_db_plugin` as the default ABI serializer time limit is not large enough to serialize large blocks.
* Read-only mode should be used to avoid speculative execution. See [Nodscs Read Modes](../../02_usage/05_nodscs-implementation.md#nodscs-read-modes). Forked data is still recorded (data that never becomes irreversible) but speculative transaction processing and signaling is avoided, minimizing the transaction_traces/action_traces stored.
...
```console
  -q [ --mongodb-queue-size ] arg (=256)
                                        The target queue size between nodscs
  ...
  -m [ --mongodb-uri ] arg              MongoDB URI connection string, see:
                                        ...
                                        database 'scs' is used if not specified
...
```console
mongodb-filter-out = lcscs:onblock:
...
* `accounts` - created on applied transaction. Always updated even if `mongodb-store-action-traces=false`.
  ...
  * Invalid ABI on account will prevent conversion of action data into JSON for storage resulting in just the action data being stored as hex. For example, the original lcscs.system contract did not provide ABI for the `onblock` action and therefore all `onblock` action data is stored as hex until the time `onblock` ABI is added to the lcscs.system contract.

* `action_traces` - created on applied transaction
  * `receipt` - action_trace action_receipt - see `lcscs::chain::action_receipt`
  * `trx_id` - transaction id
  * `act` - action - see `lcscs::chain::action`
...
* `block_states` - created on accepted block
  * `block_num`
  * `block_id`
  * `block_header_state` - see `lcscs::chain::block_header_state`
  * `validated`
  * `in_current_chain`
...
* `blocks` - created on accepted block
  * `block_num`
  * `block_id`
  * `block` - signed block - see `lcscs::chain::signed_block`
...
* `transaction_traces` - created on applied transaction
  * see `chain::lcscs::transaction_trace`

* `transactions` - created on accepted transaction - does not include inline actions
  * see `lcscs::chain::signed_transaction`. In addition to signed_transaction data the following are also stored.
  * `trx_id` - transaction id
  * `irreversible=true` - added on irreversible block
...
```console
db.pub_keys.find({"public_key":"scs7EarnUhcyYqmdnPon8rm7mBCTnBoot6o7fE2WzjvEX2TdggbL3"}).pretty()
```
