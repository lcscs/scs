...
The `http_plugin` is a core plugin supported by both `nodscs` and `kscsd`. The plugin is required to enable any RPC API functionality provided by a `nodscs` or `kscsd` instance.
...
```sh
# command-line
nodscs ... --plugin lcscs::http_plugin [options]
 (or)
kscsd ... --plugin lcscs::http_plugin [options]
```
...
Config Options for lcscs::http_plugin:
  --unix-socket-path arg                The filename (relative to data-dir) to 
                                        ...
                                        blank to disable (=kscsd.sock for kscsd)
  --http-server-address arg (=127.0.0.1:8888 for nodscs)
