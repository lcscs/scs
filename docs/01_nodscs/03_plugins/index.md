---
content_title: Nodscs Plugins
---

## Overview

Plugins extend the core functionality implemented in `nodscs`. Some plugins are mandatory, such as `chain_plugin`, `net_plugin`, and `producer_plugin`, which reflect the modular design of `nodscs`. The other plugins are optional as they provide nice to have features, but non-essential for the nodes operation.
...
[[info | Nodscs is modular]]
| Plugins add incremental functionality to `nodscs`. Unlike runtime plugins, `nodscs` plugins are built at compile-time.
