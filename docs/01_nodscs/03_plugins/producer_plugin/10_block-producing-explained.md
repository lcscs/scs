...
peer validation for similar hardware/lcscs-version/config will be <= m
...
#       * image will be rendered by gatsby-remark-graphviz plugin in lcscs docs.
...
Running wasm-runtime=scs-vm-jit scs-vm-oc-enable on relay node will reduce the validation time.
