...
This section describes how to set up a multi-node blockchain configuration running on a single host.  This is referred to as a _**single host, multi-node testnet**_.  We will set up two nodes on your local computer and have them communicate with each other.  The examples in this section rely on three command-line applications, `nodscs`, `kscsd`, and `clscs`.  The following diagram depicts the desired testnet configuration.
...
3. [Loading the lcscs Key](#3-loading-the-lcscs-key)
...
In the first terminal window, start `kscsd`, the wallet management application:

```sh
kscsd --http-server-address 127.0.0.1:8899
```

If successful, `kscsd` will display some information, starting with:
...
Look for a line saying the wallet is listening on 127.0.0.1:8899. This will indicate that `kscsd` started correctly and is listening on the correct port. If you see anything else, or you see some error report prior to "starting wallet_api_plugin", then you need to diagnose the issue and restart.

When `kscsd` is running correctly, leave that window open with the wallet app running and move to the next terminal window.

### 2. Create a Default Wallet

In the next terminal window, use `clscs`, the command-line utility, to create the default wallet.

```sh
clscs --wallet-url http://127.0.0.1:8899  wallet create --to-console
```

`clscs` will indicate that it created the "default" wallet, and will provide a password for future wallet access. As the message says, be sure to preserve this password for future use. Here is an example of this output:
...
`kscsd` will generate some status output in its window. We will continue to use this second window for subsequent `clscs` commands.

### 3. Loading the lcscs Key
...
```sh
clscs --wallet-url http://127.0.0.1:8899 wallet import --private-key 5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3
```

```console
imported private key for: scs6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
```
...
```sh
nodscs --enable-stale-production --producer-name lcscs --plugin lcscs::chain_api_plugin --plugin lcscs::net_api_plugin
```

This creates a special producer, known as the "bios" producer. Assuming everything has executed correctly to this point, you should see output from the `nodscs` process reporting block creation.

### 5. Start the Second Producer Node

The following commands assume that you are running this tutorial from the `scs\build` directory, from which you ran `./lcscs_build.sh` to build the lcscs binaries.

To start additional nodes, you must first load the `lcscs.bios` contract. This contract enables you to have direct control over the resource allocation of other accounts and to access other privileged API calls. Return to the second terminal window and run the following command to load the contract:

```sh
clscs --wallet-url http://127.0.0.1:8899 set contract lcscs build/contracts/lcscs.bios
```
...
[[caution | Caution]]
| The command line instructions that follow use the private/public keys shown below. In order to be able to cut-and-paste the command line instructions directly from this tutorial, use those keys instead of the ones generated from your `clscs create key` command. If you still want to use your newly generated keys, you need to replace the key values with yours in the commands that follow.
...
Public key: scs6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg
```
...
```sh
clscs --wallet-url http://127.0.0.1:8899 wallet import 5JgbL2ZnoEAhTudReWH1RnMuQS6DBeLZt4ucV6t8aymVEuYg7sr
```

```console
imported private key for: scs6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg
```
...
```sh
clscs --wallet-url http://127.0.0.1:8899 create account lcscs inita scs6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg scs6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg
```

```console
executed transaction: d1ea511977803d2d88f46deb554f5b6cce355b9cc3174bec0da45fc16fe9d5f3  352 bytes  102400 cycles
#         lcscs <= lcscs::newaccount            {"creator":"lcscs","name":"inita","owner":{"threshold":1,"keys":[{"key":"scs6hMjoWRF2L8x9YpeqtUEcsDK...
```
...
In the fourth terminal window, start a second `nodscs` instance. Notice that this command line is substantially longer than the one we used above to create the first producer. This is necessary to avoid collisions with the first `nodscs` instance. Fortunately, you can just cut and paste this command line and adjust the keys:

```sh
nodscs --producer-name inita --plugin lcscs::chain_api_plugin --plugin lcscs::net_api_plugin --http-server-address 127.0.0.1:8889 --p2p-listen-endpoint 127.0.0.1:9877 --p2p-peer-address 127.0.0.1:9876 --config-dir node2 --data-dir node2 --private-key [\"scs6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg\",\"5JgbL2ZnoEAhTudReWH1RnMuQS6DBeLZt4ucV6t8aymVEuYg7sr\"]
```
...
2396002ms thread-0   producer_plugin.cpp:244       block_production_loo ] Not producing block because it isn't my turn, its lcscs
```

At this point, the second `nodscs` is an idle producer. To turn it into an active producer, `inita` needs to be registered as a producer with the bios node, and the bios node needs to perform an action to update the producer schedule.

```sh
clscs --wallet-url http://127.0.0.1:8899 push action lcscs setprods "{ \"schedule\": [{\"producer_name\": \"inita\",\"block_signing_key\": \"scs6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg\"}]}" -p lcscs@active
```

```console
executed transaction: 2cff4d96814752aefaf9908a7650e867dab74af02253ae7d34672abb9c58235a  272 bytes  105472 cycles
#         lcscs <= lcscs::setprods              {"version":1,"producers":[{"producer_name":"inita","block_signing_key":"scs6hMjoWRF2L8x9YpeqtUEcsDKA...
```
...
```sh
clscs --url http://127.0.0.1:8889 get info
```
