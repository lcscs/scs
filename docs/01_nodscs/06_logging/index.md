---
content_title: Nodscs Logging
---

Logging for `nodscs` is controlled by the `logging.json` file. CLI options can be passed to `nodscs` to [setup `logging.json`](00_setup-logging.json.md). The logging configuration file can be used to define [appenders](#appenders) and tie them to [loggers](#loggers) and [logging levels](01_logging-levels.md).

## Appenders

The logging library built into lcscs supports two appender types:
...
The logging library built into lcscs currently supports the following loggers:
