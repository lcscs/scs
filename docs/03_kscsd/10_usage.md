---
content_title: Kscsd Usage
---

[[info | Recommended Usage]]
| For most users, the easiest way to use `kscsd` is to have `clscs` launch it automatically. Wallet files will be created in the default directory (`~/lcscs-wallet`).

## Launching kscsd manually

`kscsd` can be launched manually from the terminal by running:

```sh
kscsd
```

By default, `kscsd` creates the folder `~/lcscs-wallet` and populates it with a basic `config.ini` file.  The location of the config file can be specified on the command line using the `--config-dir` argument.  The configuration file contains the HTTP server endpoint for incoming HTTP connections and other parameters for cross-origin resource sharing.
...
By default, `kscsd` is set to lock your wallet after 15 minutes of inactivity. This is configurable in the `config.ini` by setting the timeout seconds in `unlock-timeout`. Setting it to 0 will cause `kscsd` to always lock your wallet.

## Stopping kscsd

The most effective way to stop `kscsd` is to find the kscsd process and send a SIGTERM signal to it.

## Other options

For a list of all commands known to `kscsd`, simply run it with no arguments:
...
Config Options for lcscs::http_plugin:
  --unix-socket-path arg (=kscsd.sock)  The filename (relative to data-dir) to
...
Config Options for lcscs::wallet_plugin:
