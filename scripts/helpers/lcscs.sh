...
if [[ -z "${NAME}" ]]; then
    ...
    else echo " ${COLOR_RED}- lcscs is not supported for your Architecture!${COLOR_NC}" && exit 1
    fi
fi

# Setup yum and apt variables
if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
    if ! YUM=$( command -v yum 2>/dev/null ); then echo "${COLOR_RED}YUM must be installed to compile lcscs${COLOR_NC}" && exit 1; fi
elif [[ $NAME == "Ubuntu" ]]; then
    if ! APTGET=$( command -v apt-get 2>/dev/null ); then echo "${COLOR_RED}APT-GET must be installed to compile lcscs${COLOR_NC}" && exit 1; fi
fi
...
function setup() {
    if $VERBOSE; then
        ...
        echo "lcscs_INSTALL_DIR: ${lcscs_INSTALL_DIR}"
...
function ensure-which() {
  if ! which ls &>/dev/null; then
    while true; do
      [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}lcscs compiler checks require the 'which' package: Would you like for us to install it? (y/n)?${COLOR_NC}" && read -p " " PROCEED
      echo ""
      case $PROCEED in
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        echo "No installation location was specified. Please provide the location where lcscs is installed."
        while true; do
            [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to use the default location? ${lcscs_INSTALL_DIR}? (y/n)${COLOR_NC}" && read -p " " PROCEED
            echo ""
            case $PROCEED in
                "" )
                echo "What would you like to do?";;
                ...
                break;;
                1 | false | [Nn]* )
                printf "Enter the desired installation location." && read -p " " lcscs_INSTALL_DIR;
                export lcscs_INSTALL_DIR;
                break;;
                * ) echo "Please type 'y' for yes or 'n' for no.";;
            esac
        done
    else
        # Support relative paths : https://github.com/lcscs/scs/issues/7560
        [[ ! $INSTALL_LOCATION =~ ^\/ ]] && export INSTALL_LOCATION="${CURRENT_WORKING_DIR}/$INSTALL_LOCATION"
        export lcscs_INSTALL_DIR="$INSTALL_LOCATION"
    fi
    . ./scripts/.build_vars
    echo "lcscs will be installed to: ${lcscs_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $lcscs_INSTALL_DIR ]]; then
    echo "lcscs has already been installed into ${lcscs_INSTALL_DIR}... It's suggested that you lcscs_uninstall.sh before re-running this script."
    while true; do
...
function resources() {
    echo "${COLOR_CYAN}lcscs website:${COLOR_NC} https://scs.io"
    echo "${COLOR_CYAN}lcscs Telegram channel:${COLOR_NC} https://t.me/scsProject"
    echo "${COLOR_CYAN}lcscs resources:${COLOR_NC} https://scs.io/resources/"
    echo "${COLOR_CYAN}lcscs Stack Exchange:${COLOR_NC} https://lcscs.stackexchange.com"
}

function print_supported_linux_distros_and_exit() {
   echo "On Linux the lcscs build script only supports Amazon, Centos, and Ubuntu."
...
function ensure-compiler() {
    ...
    if $PIN_COMPILER || [[ -f $CLANG_ROOT/bin/clang++ ]]; then
        ...
        if [[ $CXX =~ "clang" ]] || [[ $COMPILER_TYPE =~ "clang" ]]; then
            ...
            if [[ $NO_CPP17 == false ]]; then # https://github.com/lcscs/scs/issues/7402
                while true; do
                    echo "${COLOR_YELLOW}WARNING: Your GCC compiler ($CXX) is less performant than clang (https://github.com/lcscs/scs/issues/7402). We suggest running the build script with -P or install your own clang and try again.${COLOR_NC}"
                    [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to proceed anyway? (y/n)?${COLOR_NC}" && read -p " " PROCEED
                    case $PROCEED in
                        "" ) echo "What would you like to do?";;
                        0 | true | [Yy]* ) break;;
...
function ensure-cmake() {
    ...
    if [[ ! -e "${CMAKE}" ]]; then
        ...
        && ./bootstrap --prefix=${lcscs_INSTALL_DIR} \
        && make -j${JOBS} \
        && make install \
        && cd .. \
