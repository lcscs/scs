#!/bin/bash
#
# lcscs-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# lcscs-tn_bounce.sh and lcscs-tn_roll.sh scripts.

connected="0"

rundir=programs/nodscs
prog=nodscs
...
datadir=var/lib/node_$lcscs_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/lcscs/node_$lcscs_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/lcscs/node_$lcscs_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
