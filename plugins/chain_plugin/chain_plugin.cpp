...
#include <lcscs/chain/fork_database.hpp>
#include <lcscs/chain/block_log.hpp>
#include <lcscs/chain/exceptions.hpp>
#include <lcscs/chain/authorization_manager.hpp>
#include <lcscs/chain/code_object.hpp>
#include <lcscs/chain/config.hpp>
#include <lcscs/chain/wasm_interface.hpp>
#include <lcscs/chain/resource_limits.hpp>
#include <lcscs/chain/reversible_block_object.hpp>
#include <lcscs/chain/controller.hpp>
#include <lcscs/chain/generated_transaction_object.hpp>
#include <lcscs/chain/global_property_object.hpp>
#include <lcscs/chain/snapshot.hpp>

#include <lcscs/chain/lcscs_contract.hpp>
...
std::ostream& operator<<(std::ostream& osm, lcscs::chain::db_read_mode m) {
   if ( m == lcscs::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == lcscs::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == lcscs::chain::db_read_mode::READ_ONLY ) { // deprecated
      osm << "read-only";
   } else if ( m == lcscs::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              lcscs::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(lcscs::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(lcscs::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(lcscs::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(lcscs::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, lcscs::chain::validation_mode m) {
   if ( m == lcscs::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == lcscs::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              lcscs::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(lcscs::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(lcscs::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace lcscs::chain;
using namespace lcscs::chain::config;
using namespace lcscs::chain::plugin_interface;
...
:my(new chain_plugin_impl()) {
   app().register_config_type<lcscs::chain::db_read_mode>();
   app().register_config_type<lcscs::chain::validation_mode>();
   app().register_config_type<chainbase::pinnable_mapped_file::map_mode>();
   app().register_config_type<lcscs::chain::wasm_interface::vm_type>();
}
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
...
#ifdef lcscs_scs_VM_JIT_RUNTIME_ENABLED
   wasm_runtime_opt += delim + " \"scs-vm-jit\"";
   wasm_runtime_desc += "\"scs-vm-jit\" : A WebAssembly runtime that compiles WebAssembly code to native x86 code prior to execution.\n";
#endif

#ifdef lcscs_scs_VM_RUNTIME_ENABLED
   wasm_runtime_opt += delim + "\"scs-vm\"";
   wasm_runtime_desc += "\"scs-vm\" : A WebAssembly interpreter.\n";
#endif

#ifdef lcscs_scs_VM_OC_DEVELOPER
   wasm_runtime_opt += delim + "\"scs-vm-oc\"";
   wasm_runtime_desc += "\"scs-vm-oc\" : Unsupported. Instead, use one of the other runtimes along with the option enable-scs-vm-oc.\n";
#endif
   wasm_runtime_opt += ")\n" + wasm_runtime_desc;

   std::string default_wasm_runtime_str= lcscs::chain::wasm_interface::vm_type_string(lcscs::chain::config::default_wasm_runtime);
   ...
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<lcscs::chain::wasm_interface::vm_type>()->value_name("runtime")->notifier([](const auto& vm){
#ifndef lcscs_scs_VM_OC_DEVELOPER
            //throwing an exception here (like scs_ASSERT) is just gobbled up with a "Failed to initialize" error :(
            if(vm == wasm_interface::vm_type::scs_vm_oc) {
               elog("scs VM OC is a tier-up compiler and works in conjunction with the configured base WASM runtime. Enable scs VM OC via 'scs-vm-oc-enable' option");
               scs_ASSERT(false, plugin_exception, "");
            }
#endif
         })->default_value(lcscs::chain::config::default_wasm_runtime, default_wasm_runtime_str), wasm_runtime_opt.c_str()
         )
         ...
         ("read-mode", boost::program_options::value<lcscs::chain::db_read_mode>()->default_value(lcscs::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<lcscs::chain::validation_mode>()->default_value(lcscs::chain::validation_mode::FULL),
         ...
         ("scs-vm-oc-cache-size-mb", bpo::value<uint64_t>()->default_value(scsvmoc::config().cache_size / (1024u*1024u)), "Maximum size (in MiB) of the scs VM OC code cache")
         ("scs-vm-oc-compile-threads", bpo::value<uint64_t>()->default_value(1u)->notifier([](const auto t) {
               if(t == 0) {
                  elog("scs-vm-oc-compile-threads must be set to a non-zero value");
                  scs_ASSERT(false, plugin_exception, "");
               }
         }), "Number of threads to use for scs VM OC tier-up")
         ("scs-vm-oc-enable", bpo::bool_switch(), "Enable scs VM OC tier-up runtime")
#endif
...
if( options.count(op_name) ) { \
   ...
   for( const auto& v : ops ) { \
      container.emplace( lcscs::chain::name( v ) ); \
   } \
}
...
protocol_feature_set initialize_protocol_features( const fc::path& p, bool populate_missing_builtins = true ) {
   ...
   if( fc::exists( p ) ) {
      scs_ASSERT( fc::is_directory( p ), plugin_exception,
   ...
   if( directory_exists ) {
      for( directory_iterator enditr, itr{p}; itr != enditr; ++itr ) {
         ...
         scs_ASSERT( res.second, plugin_exception,
   ...
   auto output_protocol_feature = [&p]( const builtin_protocol_feature& f, const digest_type& feature_digest ) {
      ...
      scs_ASSERT( !fc::exists( file_path ), plugin_exception,
   ...
   ( builtin_protocol_feature_t codename ) -> digest_type {
      ...
      if( !res.second ) {
         scs_ASSERT( res.first->second, protocol_feature_exception,
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if lcscs_ROOT_KEY is bad
      } catch ( const fc::exception& ) {
         elog( "lcscs_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::lcscs_root_key));
         throw;
      }
      ...
      if( options.at( "print-build-info" ).as<bool>() || options.count( "extract-build-info") ) {
         ...
         if( options.count( "extract-build-info") ) {
            ...
            scs_ASSERT( fc::json::save_to_file( chainbase::environment(), p, true ), misc_exception,
         ...
         scs_THROW( node_management_success, "reported build environment information" );
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            scs_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            ...
            if( itr != my->loaded_checkpoints.end() ) {
               scs_ASSERT( itr->second == item.second,
      ...
      if( options.count( "chain-threads" )) {
         ...
         scs_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      scs_ASSERT( my->chain_config->sig_cpu_bill_pct >= 0 && my->chain_config->sig_cpu_bill_pct <= 100, plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         if( options.count( "extract-genesis-json" )) {
            ...
            scs_ASSERT( fc::json::save_to_file( *gs, p, true ),
                        misc_exception,
         ...
         scs_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if( options.count("export-reversible-blocks") ) {
         ...
         scs_THROW( node_management_success, "exported reversible blocks" );
      }
      ...
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         scs_THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
         ...
         scs_THROW( node_management_success, "imported reversible blocks" );
      }
      ...
      if (options.count( "snapshot" )) {
         ...
         scs_ASSERT( fc::exists(*my->snapshot_path), plugin_config_exception,
         ...
         scs_ASSERT( !fc::is_regular_file(shared_mem_path),
                 plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" )) {
            ...
            if( block_log_genesis ) {
               ...
               scs_ASSERT( *chain_id == block_log_chain_id,
                           plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" ) ) {
            ...
            if( chain_id ) {
               scs_ASSERT( *block_log_chain_id == *chain_id, block_log_exception,
         ...
         if( options.count( "genesis-json" ) ) {
            ...
            scs_ASSERT( fc::is_regular_file( genesis_file ),
                        plugin_config_exception,
            ...
            if( block_log_genesis ) {
               scs_ASSERT( *block_log_genesis == provided_genesis, plugin_config_exception,
               ...
               if( chain_id ) {
                  scs_ASSERT( provided_genesis_chain_id == *chain_id, plugin_config_exception,
                  ...
                  if( block_log_chain_id ) {
                     scs_ASSERT( provided_genesis_chain_id == *block_log_chain_id, plugin_config_exception,
                                 "Genesis state, provided via command line arguments, has a chain ID (${provided_genesis_chain_id}) "
         ...
         if( !chain_id ) {
            if( my->genesis ) {
               ...
               scs_ASSERT( !block_log_chain_id, plugin_config_exception,
...
#ifdef lcscs_scs_VM_OC_RUNTIME_ENABLED
      if( options.count("scs-vm-oc-cache-size-mb") )
         my->chain_config->scsvmoc_config.cache_size = options.at( "scs-vm-oc-cache-size-mb" ).as<uint64_t>() * 1024u * 1024u;
      if( options.count("scs-vm-oc-compile-threads") )
         my->chain_config->scsvmoc_config.threads = options.at("scs-vm-oc-compile-threads").as<uint64_t>();
      if( options["scs-vm-oc-enable"].as<bool>() )
         my->chain_config->scsvmoc_tierup = true;
#endif
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            auto id = blk->id();
            scs_ASSERT( itr->second == id, checkpoint_exception,
...
{ try {
   scs_ASSERT( my->chain_config->read_mode != db_read_mode::IRREVERSIBLE || !accept_transactions(), plugin_config_exception,
...
void chain_apis::read_write::validate() const {
   scs_ASSERT( api_accept_transactions, missing_chain_api_plugin_exception,
...
bool chain_plugin::recover_reversible_blocks( const fc::path& db_dir, uint32_t cache_size,
   ...
   if( new_db_dir ) {
      ...
      scs_ASSERT( reversible_dir_name != ".", invalid_reversible_blocks_dir, "Invalid path to reversible directory" );
      backup_dir = reversible_dir.parent_path() / reversible_dir_name.append("-").append( now );

      scs_ASSERT( !fc::exists(backup_dir),
   ...
   try {
      for( ; itr != ubi.end(); ++itr ) {
         scs_ASSERT( itr->blocknum == end + 1, gap_in_reversible_blocks_db,
   ...
   try {
      while( reversible_blocks.tellg() < end_pos ) {
         ...
         if( start == 0 ) {
            ...
            scs_ASSERT( num == end + 1, gap_in_reversible_blocks_db,
...
void chain_plugin::handle_guard_exception(const chain::guard_exception& e) {
   ...
   elog("database chain::guard_exception, quiting..."); // log string searched for in: tests/nodscs_under_min_avail_ram.py
   // quit the app
   app().quit();
}
...
void chain_plugin::handle_db_exhaustion() {
   ...
   //return 1 -- it's what programs/nodscs/main.cpp considers "BAD_ALLOC"
   std::_Exit(1);
}

void chain_plugin::handle_bad_alloc() {
   elog("std::bad_alloc - memory exhausted");
   //return -2 -- it's what programs/nodscs/main.cpp reports for std::exception
   std::_Exit(-2);
}
...
uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
   ...
   scs_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         ...
         scs_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   if (str.find(',') != string::npos) { // fix #6274 only match formats like 4,scs
      try {
         auto symb = lcscs::chain::symbol::from_string(str);
         return symb.value();
      } catch( ... ) { }
   }

   try {
      return ( lcscs::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
   } catch( ... ) {
      scs_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
double convert_to_type(const string& str, const string& desc) {
   ...
   scs_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
...
string convert_to_string(const chain::key256_t& source, const string& key_type, const string& encode_type, const string& desc) {
   try {
      ...
      scs_ASSERT( false, chain_type_exception, "Incompatible key_type and encode_type for key256_t next_key" );
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   scs_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   scs_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}

read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = lcscs::chain_apis::get_abi( db, p.code );
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
   bool primary = false;
   auto table_with_index = get_table_index_name( p, primary );
   if( primary ) {
      scs_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_rows_ex<key_value_index>(p,abi);
      }
      scs_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      scs_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      scs_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_key_value_table(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      scs_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      scs_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( lcscs::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_key_value_table(p.code, name(scope), N(stat), [&](const key_value_object& obj){
      scs_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_us, bool shorten_abi_errors ) {
   ...
   scs_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));
   ...
   scs_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   scs_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const try {
   const abi_def abi = lcscs::chain_apis::get_abi(db, config::system_account_name);
   ...
   scs_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   scs_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  scs_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   scs_ASSERT( !params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64,
   ...
   if( block_num.valid() ) {
      ...
      } scs_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))
   }

   scs_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   ...
   scs_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   try {
      ...
      } scs_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   try {
      scs_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
read_only::get_code_results read_only::get_code( const get_code_params& params )const {
   ...
   scs_ASSERT( params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported" );
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   while( perm != permissions.end() && perm->owner == params.account_name ) {
      ...
      if( perm->parent._id ) {
         ...
         if( p ) {
            scs_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
            parent = p->name;
         }
      }
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(scs.token);
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   scs_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer::create_yield_function( abi_serializer_max_time ) );
      auto action_type = abis.get_action_type(params.action);
      scs_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
      ...
      } scs_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      scs_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } scs_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
...
{
   scs_ASSERT(aqdb.valid(), plugin_config_exception, "Account Queries being accessed when not enabled");
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   // The following code makes assumptions about the contract deployed on lcscs account (i.e. the system contract) and how it stores its data.
   const auto& d = db.db();
   const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( N(lcscs), N(lcscs), N(rammarket) ));
   if( t_id != nullptr ) {
      const auto &idx = d.get_index<key_value_index, by_scope_primary>();
      auto it = idx.find(boost::make_tuple( t_id->id, lcscs::chain::string_to_symbol_c(4,"RAMCORE") ));
      if( it != idx.end() ) {
...
FC_REFLECT( lcscs::chain_apis::detail::ram_market_exchange_state_t, (ignore1)(ignore2)(ignore3)(core_symbol)(ignore4) )
