#include <lcscs/producer_plugin/producer_plugin.hpp>
#include <lcscs/chain/plugin_interface.hpp>
...
#include <lcscs/chain/unapplied_transaction_queue.hpp>
...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      fc::microseconds                                          _kscsd_provider_timeout_us;
      ...
      bool on_incoming_block(const signed_block_ptr& block, const std::optional<block_id_type>& block_id) {
         ...
         scs_ASSERT( block->timestamp < (fc::time_point::now() + fc::seconds( 7 )), block_from_the_future,
      ...
      private:
         ...
         void add_size( const transaction_metadata_ptr& trx ) {
            ...
            scs_ASSERT( size_in_bytes + size < max_incoming_transaction_queue_size, tx_resource_exhaustion, "Transaction exceeded producer resource limit" );
            size_in_bytes += size;
         }
      ...
      public:
         ...
         auto pop_front() {
            scs_ASSERT( !_incoming_transactions.empty(), producer_exception, "logic error, front() called on empty incoming_transactions" );
...
void new_chain_banner(const lcscs::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ...
         ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value(
          ...
          "Where:\n"
          "   <public-key>    \tis a string form of a vaild lcscs public key\n\n"
          "   <provider-spec> \tis a string in the form <provider-type>:<data>\n\n"
          "   <provider-type> \tis KEY, or KscsD\n\n"
          "   KEY:<data>      \tis a string form of a valid lcscs private key which maps to the provided public key\n\n"
          "   KscsD:<data>    \tis the URL where kscsd is available and the approptiate wallet(s) are unlocked")
         ("kscsd-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
          "Limits the maximum time (in milliseconds) that is allowed for sending blocks to a kscsd provider for signing")
...
chain::signature_type producer_plugin::sign_compact(const chain::public_key_type& key, const fc::sha256& digest) const
{
  if(key != chain::public_key_type()) {
    auto private_key_itr = my->_signature_providers.find(key);
    scs_ASSERT(private_key_itr != my->_signature_providers.end(), producer_priv_key_not_found, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
static producer_plugin_impl::signature_provider_type
make_kscsd_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   fc::url kscsd_url;
   if(boost::algorithm::starts_with(url_str, "unix://"))
      ...
      kscsd_url = fc::url("unix", url_str.substr(7), ostring(), ostring(), ostring(), ostring(), ovariant_object(), fc::optional<uint16_t>());
   else
      kscsd_url = fc::url(url_str);
   std::weak_ptr<producer_plugin_impl> weak_impl = impl;

   return [weak_impl, kscsd_url, pubkey]( const chain::digest_type& digest ) {
      auto impl = weak_impl.lock();
      if (impl) {
         fc::variant params;
         fc::to_variant(std::make_pair(digest, pubkey), params);
         auto deadline = impl->_kscsd_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_kscsd_provider_timeout_us : fc::time_point::maximum();
         return app().get_plugin<http_client_plugin>().get_client().post_sync(kscsd_url, params, deadline).as<chain::signature_type>();
      } else {
...
{ try {
   ...
   scs_ASSERT( my->chain_plug, plugin_config_exception, "chain_plugin not found" );
   my->_options = &options;
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            scs_ASSERT(delim != std::string::npos, plugin_config_exception, "Missing \"=\" in the key spec pair");
            auto pub_key_str = key_spec_pair.substr(0, delim);
            auto spec_str = key_spec_pair.substr(delim + 1);

            auto spec_delim = spec_str.find(":");
            scs_ASSERT(spec_delim != std::string::npos, plugin_config_exception, "Missing \":\" in the key spec pair");
            ...
            } else if (spec_type_str == "KscsD") {
               my->_signature_providers[pubkey] = make_kscsd_signature_provider(my, spec_data, pubkey);
            }
   ...
   my->_kscsd_provider_timeout_us = fc::milliseconds(options.at("kscsd-provider-timeout").as<int32_t>());

   my->_produce_time_offset_us = options.at("produce-time-offset-us").as<int32_t>();
   scs_ASSERT( my->_produce_time_offset_us <= 0 && my->_produce_time_offset_us >= -config::block_interval_us, plugin_config_exception,
   ...
   scs_ASSERT( my->_last_block_time_offset_us <= 0 && my->_last_block_time_offset_us >= -config::block_interval_us, plugin_config_exception,
   ...
   scs_ASSERT( cpu_effort_pct >= 0 && cpu_effort_pct <= 100, plugin_config_exception,
               "cpu-effort-percent ${pct} must be 0 - 100", ("pct", cpu_effort_pct) );
      cpu_effort_pct *= config::percent_1;
   int32_t cpu_effort_offset_us =
         -scs_PERCENT( config::block_interval_us, chain::config::percent_100 - cpu_effort_pct );

   uint32_t last_block_cpu_effort_pct = options.at("last-block-cpu-effort-percent").as<uint32_t>();
   scs_ASSERT( last_block_cpu_effort_pct >= 0 && last_block_cpu_effort_pct <= 100, plugin_config_exception,
   ...
   int32_t last_block_cpu_effort_offset_us =
         -scs_PERCENT( config::block_interval_us, chain::config::percent_100 - last_block_cpu_effort_pct );
   ...
   scs_ASSERT( my->_max_block_cpu_usage_threshold_us < config::block_interval_us, plugin_config_exception,
   ...
   scs_ASSERT( max_incoming_transaction_queue_size > 0, plugin_config_exception,
   ...
   scs_ASSERT( thread_pool_size > 0, plugin_config_exception,
   ...
   if( options.count( "snapshots-dir" )) {
      ...
      scs_ASSERT( fc::is_directory(my->_snapshots_dir), snapshot_directory_not_found_exception,
...
{ try {
   ...
   chain::controller& chain = my->chain_plug->chain();
   scs_ASSERT( my->_producers.empty() || chain.get_read_mode() == chain::db_read_mode::SPECULATIVE, plugin_config_exception,
              "node cannot have any producer-name configured because block production is impossible when read_mode is not \"speculative\"" );

   scs_ASSERT( my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, plugin_config_exception,
              "node cannot have any producer-name configured because block production is not safe when validation_mode is not \"full\"" );

   scs_ASSERT( my->_producers.empty() || my->chain_plug->accept_transactions(), plugin_config_exception,
...
void producer_plugin::schedule_protocol_feature_activations( const scheduled_protocol_feature_activations& schedule ) {
   ...
   scs_ASSERT( set_of_features_to_activate.size() == schedule.protocol_features_to_activate.size(),
   ...
   for (auto &feature_digest : set_of_features_to_activate) {
      ...
      scs_ASSERT( !pf.preactivation_required, protocol_feature_exception,
...
void producer_plugin_impl::schedule_production_loop() {
   ...
   if (result == start_block_result::failed) {
      ...
      scs_ASSERT( chain.is_building_block(), missing_pending_block_state, "speculating without pending_block_state" );
...
void producer_plugin_impl::schedule_maybe_produce_block( bool exhausted ) {
   ...
   if( !exhausted && deadline > fc::time_point::now() ) {
      ...
      scs_ASSERT( chain.is_building_block(), missing_pending_block_state,
      ...
      scs_ASSERT( chain.is_building_block(), missing_pending_block_state, "producing without pending_block_state" );
...
void producer_plugin_impl::produce_block() {
   ...
   scs_ASSERT(_pending_block_mode == pending_block_mode::producing, producer_exception, "called produce_block while not actually producing");
   chain::controller& chain = chain_plug->chain();
   const auto& hbs = chain.head_block_state();
   scs_ASSERT(chain.is_building_block(), missing_pending_block_state, "pending_block_state does not exist but it should, another plugin may have corrupted it");
   ...
   scs_ASSERT(relevant_providers.size() > 0, producer_priv_key_not_found, "Attempting to produce a block for which we don't have any relevant private keys");
