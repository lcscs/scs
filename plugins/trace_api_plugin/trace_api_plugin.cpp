#include <lcscs/trace_api/trace_api_plugin.hpp>

#include <lcscs/trace_api/abi_data_handler.hpp>
#include <lcscs/trace_api/request_handler.hpp>
#include <lcscs/trace_api/chain_extraction.hpp>
#include <lcscs/trace_api/store_provider.hpp>

#include <lcscs/trace_api/configuration_utils.hpp>

#include <boost/signals2/connection.hpp>

using namespace lcscs::trace_api;
using namespace lcscs::trace_api::configuration_utils;
...
namespace {
   ...
   void emit_killer(F&& f) {
      try {
         ...
         scs_THROW(chain::controller_emit_signal_exception, "Trace API encountered an Error which it cannot recover from.  Please resolve the error and relaunch the process")
      }
   }
...
struct trace_api_common_impl {
   ...
   void plugin_initialize(const appbase::variables_map& options) {
      ...
      scs_ASSERT(blocks >= -1, chain::plugin_config_exception,
      ...
      scs_ASSERT(uncompressed_blocks >= -1, chain::plugin_config_exception,
...
struct trace_api_rpc_plugin_impl : public std::enable_shared_from_this<trace_api_rpc_plugin_impl>
{
   ...
   void plugin_initialize(const appbase::variables_map& options) {
      ...
      if( options.count("trace-rpc-abi") ) {
         scs_ASSERT(options.count("trace-no-abis") == 0, chain::plugin_config_exception,
