...
namespace lcscs::trace_api::configuration_utils {
   ...
   chain::abi_def abi_def_from_file(const std::string& file_name, const fc::path& data_dir )
   {
      ...
      scs_ASSERT(fc::exists(abi_path) && !fc::is_directory(abi_path), chain::plugin_config_exception, "${path} does not exist or is not a file", ("path", abi_path.generic_string()));
      try {
         abi_variant = fc::json::from_file(abi_path);
      } scs_RETHROW_EXCEPTIONS(chain::json_parse_exception, "Fail to parse JSON from file: ${file}", ("file", abi_path.generic_string()));
   ...
   std::pair<std::string, std::string> parse_kv_pairs( const std::string& input ) {
      scs_ASSERT(!input.empty(), chain::plugin_config_exception, "Key-Value Pair is Empty");
      auto delim = input.find("=");
      scs_ASSERT(delim != std::string::npos, chain::plugin_config_exception, "Missing \"=\"");
      scs_ASSERT(delim != 0, chain::plugin_config_exception, "Missing Key");
      scs_ASSERT(delim + 1 != input.size(), chain::plugin_config_exception, "Missing Value");
