...
#include <lcscs/trace_api/test_common.hpp>
#include <lcscs/trace_api/chain_extraction.hpp>

using namespace lcscs;
using namespace lcscs::trace_api;
using namespace lcscs::trace_api::test_common;
using lcscs::chain::name;
using lcscs::chain::digest_type;
...
namespace {
   ...
   auto make_transfer_action( chain::name from, chain::name to, chain::asset quantity, std::string memo ) {
      return chain::action( std::vector<chain::permission_level> {{from, chain::config::active_name}},
                            "lcscs.token"_n, "transfer"_n, make_transfer_data( from, to, quantity, std::move(memo) ) );
   }
...
BOOST_AUTO_TEST_SUITE(block_extraction)
   ...
   BOOST_FIXTURE_TEST_CASE(basic_single_transaction_block, extraction_test_fixture)
   {
      ...
      auto actt1 = make_action_trace( 0, act1, "lcscs.token"_n );
            ...
            {
               {
                  ptrx1.id(),
                  {
                     {
                        0,
                        "lcscs.token"_n, "lcscs.token"_n, "transfer"_n,
                        {{ "alice"_n, "active"_n }},
                        make_transfer_data( "alice"_n, "bob"_n, "0.0001 SYS"_t, "Memo!" )
                     },
                     {
                        1,
                        "alice"_n, "lcscs.token"_n, "transfer"_n,
                        {{ "alice"_n, "active"_n }},
                        make_transfer_data( "alice"_n, "bob"_n, "0.0001 SYS"_t, "Memo!" )
                     },
                     {
                        2,
                        "bob"_n, "lcscs.token"_n, "transfer"_n,
                        {{ "alice"_n, "active"_n }},
                        make_transfer_data( "alice"_n, "bob"_n, "0.0001 SYS"_t, "Memo!" )
                     }
                  }
            ...
            {
               {
                  ptrx3.id(),
                  {
                     {
                        2,
                        "fred"_n, "lcscs.token"_n, "transfer"_n,
                        {{ "fred"_n, "active"_n }},
   ...
   BOOST_FIXTURE_TEST_CASE(onerror_transaction_block, extraction_test_fixture)
   {
      ...
      auto actt1 = make_action_trace( 0, onerror_act, "lcscs.token"_n );
            ...
            {
               {
                  transfer_trx.id(), // transfer_trx.id() because that is the trx id known to the user
                  {
                     {
                        0,
                        "lcscs.token"_n, "lcscs"_n, "onerror"_n,
                        {{ "alice"_n, "active"_n }},
