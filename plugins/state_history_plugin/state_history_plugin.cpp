...
#include <lcscs/state_history_plugin/state_history_log.hpp>
#include <lcscs/state_history_plugin/state_history_serialization.hpp>
...
bool include_delta(const lcscs::chain::table_id_object& old, const lcscs::chain::table_id_object& curr) {
   return old.payer != curr.payer;
}

bool include_delta(const lcscs::chain::resource_limits::resource_limits_object& old,
                   const lcscs::chain::resource_limits::resource_limits_object& curr) {
   return                                   //
...
bool include_delta(const lcscs::chain::resource_limits::resource_limits_state_object& old,
                   const lcscs::chain::resource_limits::resource_limits_state_object& curr) {
   return                                                                                       //
...
bool include_delta(const lcscs::chain::account_metadata_object& old,
                   const lcscs::chain::account_metadata_object& curr) {
   return                                               //
       old.name != curr.name ||                         //
...
bool include_delta(const lcscs::chain::code_object& old, const lcscs::chain::code_object& curr) { //
   return false;
}

bool include_delta(const lcscs::chain::protocol_state_object& old, const lcscs::chain::protocol_state_object& curr) {
...
struct state_history_plugin_impl : std::enable_shared_from_this<state_history_plugin_impl> {
   ...
   void listen() {
      ...
      auto check_ec = [&](const char* what) {
         if (!ec)
            return;
         elog("${w}: ${m}", ("w", what)("m", ec.message()));
         scs_ASSERT(false, plugin_exception, "unable to open listen socket");
      };
   ...
   void store_traces(const block_state_ptr& block_state) {
      ...
      for (auto& r : block_state->block->transactions) {
         ...
         scs_ASSERT(it != cached_traces.end() && it->second.trace->receipt, plugin_exception,
      ...
      scs_ASSERT(traces_bin.size() == (uint32_t)traces_bin.size(), plugin_exception, "traces is too big");
   ...
   void store_chain_state(const block_state_ptr& block_state) {
      ...
      auto get_table_id = [&](uint64_t tid) -> const table_id_object& {
         ...
         scs_ASSERT(it != removed_table_id.end(), chain::plugin_exception, "can not found table id ${tid}",
                    ("tid", tid));
         return *it->second;
      };
      ...
      scs_ASSERT(deltas_bin.size() == (uint32_t)deltas_bin.size(), plugin_exception, "deltas is too big");
      state_history_log_header header{.magic        = ship_magic(ship_current_version),
                                      .block_id     = block_state->block->id(),
...
void state_history_plugin::plugin_initialize(const variables_map& options) {
   try {
      scs_ASSERT(options.at("disable-replay-opts").as<bool>(), plugin_exception,
