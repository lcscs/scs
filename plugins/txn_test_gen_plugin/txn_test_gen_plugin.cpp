#include <lcscs/txn_test_gen_plugin/txn_test_gen_plugin.hpp>
...
namespace lcscs { namespace detail {
...
FC_REFLECT(lcscs::detail::txn_test_gen_empty, );
FC_REFLECT(lcscs::detail::txn_test_gen_status, (status));
...
#define INVOKE_V_R_R_R(api_handle, call_name, in_param0, in_param1, in_param2) \
     ...
     lcscs::detail::txn_test_gen_status result = { status };

#define INVOKE_V_R_R(api_handle, call_name, in_param0, in_param1) \
     const auto& vs = fc::json::json::from_string(body).as<fc::variants>(); \
     api_handle->call_name(vs.at(0).as<in_param0>(), vs.at(1).as<in_param1>()); \
     lcscs::detail::txn_test_gen_empty result;
   ...
   [this](string, string body, url_response_callback cb) mutable { \
      ...
      auto result_handler = [times_called{std::move(times_called)}, cb, body](const fc::exception_ptr& e) mutable {\
         ...
         if (e) {\
            ...
            cb(http_response_code, fc::variant(lcscs::detail::txn_test_gen_empty())); \
         }\
      };\
      INVOKE \
   }\
...
struct txn_test_gen_plugin_impl {
   ...
   fc::optional<lcscs::chain::named_thread_pool>        thread_pool;
   ...
   void create_test_accounts(const std::string& init_name, const std::string& init_priv_key, const std::function<void(const fc::exception_ptr&)>& next) {
      ...
      try {
         ...
         abi_def currency_abi_def = fc::json::from_string(contracts::lcscs_token_abi().data()).as<abi_def>();

         controller& cc = app().get_plugin<chain_plugin>().chain();
         auto chainid = app().get_plugin<chain_plugin>().get_chain_id();
         auto abi_serializer_max_time = app().get_plugin<chain_plugin>().get_abi_serializer_max_time();

         abi_serializer lcscs_token_serializer{fc::json::from_string(contracts::lcscs_token_abi().data()).as<abi_def>(),
         ...
         //create some test accounts
         {
            ...
            //create "A" account
            {
            auto owner_auth   = lcscs::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            auto active_auth  = lcscs::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            ...
            //create "B" account
            {
            auto owner_auth   = lcscs::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            auto active_auth  = lcscs::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            ...
            //create "T" account
            {
            auto owner_auth   = lcscs::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
            auto active_auth  = lcscs::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
         ...
         //set newaccountT contract to lcscs.token & initialize it
         {
            signed_transaction trx;

            vector<uint8_t> wasm = contracts::lcscs_token_wasm();
...

            {
               setabi handler;
               handler.account = newaccountT;
               handler.abi = fc::raw::pack(json::from_string(contracts::lcscs_token_abi().data()).as<abi_def>());
               trx.actions.emplace_back( vector<chain::permission_level>{{newaccountT,name("active")}}, handler);
            }

            {
               ...
               act.data = lcscs_token_serializer.variant_to_binary("create",
            ...
            }
            {
               ...
               act.name = N(issue);
               act.authorization = vector<permission_level>{{newaccountT,config::active_name}};
               act.data = lcscs_token_serializer.variant_to_binary("issue",
            ...
            }
            {
               ...
               act.name = N(transfer);
               act.authorization = vector<permission_level>{{newaccountT,config::active_name}};
               act.data = lcscs_token_serializer.variant_to_binary("transfer",
   ...
   string start_generation(const std::string& salt, const uint64_t& period, const uint64_t& batch_size) {
      ...
      controller& cc = app().get_plugin<chain_plugin>().chain();
      auto abi_serializer_max_time = app().get_plugin<chain_plugin>().get_abi_serializer_max_time();
      abi_serializer lcscs_token_serializer{fc::json::from_string(contracts::lcscs_token_abi().data()).as<abi_def>(), abi_serializer::create_yield_function( abi_serializer_max_time )};
      ...
      act_a_to_b.data = lcscs_token_serializer.variant_to_binary("transfer",
      ...
      act_b_to_a.data = lcscs_token_serializer.variant_to_binary("transfer",
                                                                  fc::json::from_string(fc::format_string("{\"from\":\"${from}\",\"to\":\"${to}\",\"quantity\":\"1.0000 CUR\",\"memo\":\"${l}\"}",
...
void txn_test_gen_plugin::plugin_initialize(const variables_map& options) {
   try {
      ...
      my->newaccountA = lcscs::chain::name(thread_pool_account_prefix + "a");
      my->newaccountB = lcscs::chain::name(thread_pool_account_prefix + "b");
      my->newaccountT = lcscs::chain::name(thread_pool_account_prefix + "t");
      scs_ASSERT( my->thread_pool_size > 0, chain::plugin_config_exception,
