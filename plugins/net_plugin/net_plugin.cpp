...
#include <lcscs/net_plugin/net_plugin.hpp>
#include <lcscs/net_plugin/protocol.hpp>
...
namespace lcscs {
   ...
   using lcscs::chain::transaction_id_type;
   using lcscs::chain::sha256_less;
   ...
   typedef multi_index_container<
      lcscs::peer_block_state,
      indexed_by<
         ordered_unique< tag<by_id>,
               composite_key< peer_block_state,
                     member<peer_block_state, uint32_t, &lcscs::peer_block_state::connection_id>,
                     member<peer_block_state, block_id_type, &lcscs::peer_block_state::id>
               >,
         ...
         ordered_non_unique< tag<by_block_id>,
               composite_key< peer_block_state,
                     member<peer_block_state, block_id_type, &lcscs::peer_block_state::id>,
                     member<peer_block_state, bool, &lcscs::peer_block_state::have_block>
               >,
               composite_key_compare< sha256_less, std::greater<bool> >
         >,
         ordered_non_unique< tag<by_block_num>, member<lcscs::peer_block_state, uint32_t, &lcscs::peer_block_state::block_num > >
      >
   ...
   public:
      ...
      optional<lcscs::chain::named_thread_pool> thread_pool;
  ...
  FC_MULTILINE_MACRO_END
   ...
   public:
      ...
      void fill_out_buffer( std::vector<boost::asio::const_buffer>& bufs ) {
         ...
         if( _sync_write_queue.size() > 0 ) { // always send msgs from sync_write_queue first
            ...
            scs_ASSERT( _write_queue_size == 0, plugin_exception, "write queue size expected to be zero" );
         }
      }
   ...
   struct msg_handler : public fc::visitor<void> {
      ...
      void operator()( const T& ) const {
         scs_ASSERT( false, plugin_config_exception, "Not implemented, call handle_message directly instead" );
      }
   ...
   void sync_manager::sync_recv_notice( const connection_ptr& c, const notice_message& msg) {
      ...
      scs_ASSERT( msg.known_blocks.mode == catch_up || msg.known_blocks.mode == last_irr_catch_up, plugin_exception,
              ...
              [conn = shared_from_this(), socket=socket]( boost::system::error_code ec, std::size_t bytes_transferred ) {
               ...
               try {
                  if( !ec ) {
                     ...
                     scs_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), plugin_exception, "");
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "p2p-peer-address", bpo::value< vector<string> >()->composing(),
           ...
           "  Examples:\n"
           "    p2p.scs.io:9876\n"
           "    p2p.trx.scs.io:9876:trx\n"
           "    p2p.blk.scs.io:9876:blk\n")
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"scs Test Agent\""), "The name supplied to identify this node amongst the peers.")
   ...
   void net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         if( options.count( "p2p-listen-endpoint" ) && options.at("p2p-listen-endpoint").as<string>().length()) {
            ...
            scs_ASSERT( my->p2p_address.length() <= max_p2p_address_length, chain::plugin_config_exception,
         ...
         if( options.count( "p2p-server-address" ) ) {
            ...
            scs_ASSERT( my->p2p_server_address.length() <= max_p2p_address_length, chain::plugin_config_exception,
         ...
         if( options.count( "agent-name" )) {
            ...
            scs_ASSERT( my->user_agent_name.length() <= max_handshake_str_length, chain::plugin_config_exception,
         ...
         if( my->allowed_connections & net_plugin_impl::Specified )
            scs_ASSERT( options.count( "peer-key" ),
         ...
         scs_ASSERT( my->chain_plug, chain::missing_chain_plugin_exception, ""  );
