#include <lcscs/chain_api_plugin/chain_api_plugin.hpp>
...
namespace {
   ...
   T parse_params(const std::string& body) {
      if (body.empty()) {
         scs_THROW(chain::invalid_http_request, "A Request body is required");
      }

      try {
        try {
           return fc::json::from_string(body).as<T>();
        } catch (const chain::chain_exception& e) { // scs_RETHROW_EXCEPTIONS does not re-type these so, re-code it
          throw fc::exception(e);
        }
      } scs_RETHROW_EXCEPTIONS(chain::invalid_http_request, "Unable to parse valid input from POST body");
   }
}
