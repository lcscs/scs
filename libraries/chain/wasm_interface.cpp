#include <lcscs/chain/wasm_interface.hpp>
#include <lcscs/chain/apply_context.hpp>
#include <lcscs/chain/controller.hpp>
#include <lcscs/chain/transaction_context.hpp>
#include <lcscs/chain/producer_schedule.hpp>
#include <lcscs/chain/exceptions.hpp>
#include <boost/core/ignore_unused.hpp>
#include <lcscs/chain/authorization_manager.hpp>
#include <lcscs/chain/resource_limits.hpp>
#include <lcscs/chain/wasm_interface_private.hpp>
#include <lcscs/chain/wasm_lcscs_validation.hpp>
#include <lcscs/chain/wasm_lcscs_injection.hpp>
#include <lcscs/chain/global_property_object.hpp>
#include <lcscs/chain/protocol_state_object.hpp>
#include <lcscs/chain/account_object.hpp>
...
#include <lcscs/vm/allocator.hpp>
#endif

namespace lcscs { namespace chain {
   using namespace webassembly::common;

   wasm_interface::wasm_interface(vm_type vm, bool scsvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const scsvmoc::config& scsvmoc_config)
     : my( new wasm_interface_impl(vm, scsvmoc_tierup, d, data_dir, scsvmoc_config) ) {}
   ...
   void wasm_interface::validate(const controller& control, const bytes& code) {
      ...
      try {
         ...
         scs_ASSERT(false, wasm_serialization_error, e.message.c_str());
...
#ifdef lcscs_scs_VM_OC_RUNTIME_ENABLED
      if(my->scsvmoc) {
         const chain::scsvmoc::code_descriptor* cd = nullptr;
         try {
            cd = my->scsvmoc->cc.get_descriptor_for_code(code_hash, vm_version);
         }
         catch(...) {
            //swallow errors here, if scs VM OC has gone in to the weeds we shouldn't bail: continue to try and run baseline
            //In the future, consider moving bits of scs VM that can fire exceptions and such out of this call path
            static bool once_is_enough;
            if(!once_is_enough)
               elog("scs VM OC has encountered an unexpected failure");
            once_is_enough = true;
         }
         if(cd) {
            my->scsvmoc->exec.execute(*cd, my->scsvmoc->mem, context);
            return;
         }
      }
#endif
   ...
   public:
      ...
      :context(ctx)
      {
         if( context.is_context_free() )
            scs_ASSERT( context_free, unaccessible_api, "only context free api's can be used in this context" );
      }
...
class context_free_api : public context_aware_api {
   public:
      ...
      :context_aware_api(ctx, true) {
         ...
         scs_ASSERT( context.is_context_free(), unaccessible_api, "this API may only be called from context_free apply" );
      }
...
class privileged_api : public context_aware_api {
   public:
      ...
      {
         scs_ASSERT( context.is_privileged(), unaccessible_api, "${code} does not have permission to call this API", ("code",context.get_receiver()) );
      }
      ...
      void activate_feature( int64_t feature_name ) {
         scs_ASSERT( false, unsupported_feature, "Unsupported Hardfork Detected" );
      }
      ...
      void set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) {
         scs_ASSERT(ram_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
         scs_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
         scs_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
      ...
      int64_t set_proposed_producers_common( vector<producer_authority> && producers, bool validate_keys ) {
         scs_ASSERT(producers.size() <= config::max_producers, wasm_execution_error, "Producer schedule exceeds the maximum producer count for this chain");
         scs_ASSERT( producers.size() > 0
         ...
         for (const auto& p: producers) {
            scs_ASSERT( context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account" );
            ...
            p.authority.visit([&p, num_supported_key_types, validate_keys](const auto& a) {
               ...
               for (const auto& kw: a.keys ) {
                  scs_ASSERT( kw.key.which() < num_supported_key_types, unactivated_key_type,
                              "Unactivated key type used in proposed producer schedule");

                  if( validate_keys ) {
                     scs_ASSERT( kw.key.valid(), wasm_execution_error, "producer schedule includes an invalid key" );
                  }
               ...
               scs_ASSERT( a.keys.size() == unique_keys.size(), wasm_execution_error, "producer schedule includes a duplicated key for ${account}", ("account", p.producer_name));
               scs_ASSERT( a.threshold > 0, wasm_execution_error, "producer schedule includes an authority with a threshold of 0 for ${account}", ("account", p.producer_name));
               scs_ASSERT( sum_weights >= a.threshold, wasm_execution_error, "producer schedule includes an unsatisfiable authority for ${account}", ("account", p.producer_name));
            });


            unique_producers.insert(p.producer_name);
         }
         scs_ASSERT( producers.size() == unique_producers.size(), wasm_execution_error, "duplicate producer name in producer schedule" );
      ...
      int64_t set_proposed_producers_ex( uint64_t packed_producer_format, array_ptr<char> packed_producer_schedule, uint32_t datalen ) {
         if (packed_producer_format == 0) {
            ...
            scs_THROW(wasm_execution_error, "Producer schedule is in an unknown format!");
         }
      }
...
#pragma GCC diagnostic pop
      ...
      float _lcscs_f32_max( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a)) {
            return af;
      ...
      float _lcscs_f32_copysign( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
      ...
      // float unops
      float _lcscs_f32_abs( float af ) {
         float32_t a = to_softfloat32(af);
         a.v &= ~(1 << 31);
         return from_softfloat32(a);
      }
      float _lcscs_f32_neg( float af ) {
         float32_t a = to_softfloat32(af);
      ...
      float _lcscs_f32_floor( float af ) {
         float32_t a = to_softfloat32(af);
         int e = (int)(a.v >> 23 & 0xFF) - 0X7F;
         uint32_t m;
         if (e >= 23)
      ...
      float _lcscs_f32_trunc( float af ) {
         float32_t a = to_softfloat32(af);
      ...
      float _lcscs_f32_nearest( float af ) {
         float32_t a = to_softfloat32(af);
         int e = a.v>>23 & 0xff;
         int s = a.v>>31;
         float32_t y;
      ...
      // float relops
      bool _lcscs_f32_eq( float a, float b ) {  return ::f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _lcscs_f32_ne( float a, float b ) { return !::f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _lcscs_f32_lt( float a, float b ) { return ::f32_lt( to_softfloat32(a), to_softfloat32(b) ); }
      bool _lcscs_f32_le( float a, float b ) { return ::f32_le( to_softfloat32(a), to_softfloat32(b) ); }
      bool _lcscs_f32_gt( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      bool _lcscs_f32_ge( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      double _lcscs_f64_max( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return af;
      ...
      double _lcscs_f64_copysign( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
      ...
      // double unops
      double _lcscs_f64_abs( double af ) {
         float64_t a = to_softfloat64(af);
         a.v &= ~(uint64_t(1) << 63);
         return from_softfloat64(a);
      }
      double _lcscs_f64_neg( double af ) {
         float64_t a = to_softfloat64(af);
      ...
      double _lcscs_f64_sqrt( double a ) {
         float64_t ret = ::f64_sqrt( to_softfloat64(a) );
         return from_softfloat64(ret);
      }
      // ceil, floor, trunc and nearest are lifted from libc
      double _lcscs_f64_ceil( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7ff;
         float64_t y;
      ...
      double _lcscs_f64_floor( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7FF;
         float64_t y;
      ...
      double _lcscs_f64_trunc( double af ) {
         float64_t a = to_softfloat64( af );
      ...
      double _lcscs_f64_nearest( double af ) {
         float64_t a = to_softfloat64( af );
         int e = (a.v >> 52 & 0x7FF);
         int s = a.v >> 63;
         float64_t y;
      ...
      // double relops
      bool _lcscs_f64_eq( double a, double b ) { return ::f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _lcscs_f64_ne( double a, double b ) { return !::f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _lcscs_f64_lt( double a, double b ) { return ::f64_lt( to_softfloat64(a), to_softfloat64(b) ); }
      bool _lcscs_f64_le( double a, double b ) { return ::f64_le( to_softfloat64(a), to_softfloat64(b) ); }
      bool _lcscs_f64_gt( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      bool _lcscs_f64_ge( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      double _lcscs_f32_promote( float a ) {
         return from_softfloat64(f32_to_f64( to_softfloat32(a)) );
      }
      float _lcscs_f64_demote( double a ) {
         return from_softfloat32(f64_to_f32( to_softfloat64(a)) );
      }
      int32_t _lcscs_f32_trunc_i32s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_lcscs_f32_ge(af, 2147483648.0f) || _lcscs_f32_lt(af, -2147483648.0f))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f32.convert_s/i32 overflow" );

         if (is_nan(a))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f32.convert_s/i32 unrepresentable");
         return f32_to_i32( to_softfloat32(_lcscs_f32_trunc( af )), 0, false );
      }
      int32_t _lcscs_f64_trunc_i32s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_lcscs_f64_ge(af, 2147483648.0) || _lcscs_f64_lt(af, -2147483648.0))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f64.convert_s/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f64.convert_s/i32 unrepresentable");
         return f64_to_i32( to_softfloat64(_lcscs_f64_trunc( af )), 0, false );
      }
      uint32_t _lcscs_f32_trunc_i32u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_lcscs_f32_ge(af, 4294967296.0f) || _lcscs_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f32.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f32.convert_u/i32 unrepresentable");
         return f32_to_ui32( to_softfloat32(_lcscs_f32_trunc( af )), 0, false );
      }
      uint32_t _lcscs_f64_trunc_i32u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_lcscs_f64_ge(af, 4294967296.0) || _lcscs_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f64.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f64.convert_u/i32 unrepresentable");
         return f64_to_ui32( to_softfloat64(_lcscs_f64_trunc( af )), 0, false );
      }
      int64_t _lcscs_f32_trunc_i64s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_lcscs_f32_ge(af, 9223372036854775808.0f) || _lcscs_f32_lt(af, -9223372036854775808.0f))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f32.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f32.convert_s/i64 unrepresentable");
         return f32_to_i64( to_softfloat32(_lcscs_f32_trunc( af )), 0, false );
      }
      int64_t _lcscs_f64_trunc_i64s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_lcscs_f64_ge(af, 9223372036854775808.0) || _lcscs_f64_lt(af, -9223372036854775808.0))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f64.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f64.convert_s/i64 unrepresentable");

         return f64_to_i64( to_softfloat64(_lcscs_f64_trunc( af )), 0, false );
      }
      uint64_t _lcscs_f32_trunc_i64u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_lcscs_f32_ge(af, 18446744073709551616.0f) || _lcscs_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f32.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f32.convert_u/i64 unrepresentable");
         return f32_to_ui64( to_softfloat32(_lcscs_f32_trunc( af )), 0, false );
      }
      uint64_t _lcscs_f64_trunc_i64u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_lcscs_f64_ge(af, 18446744073709551616.0) || _lcscs_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f64.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( lcscs::chain::wasm_execution_error, "Error, f64.convert_u/i64 unrepresentable");
         return f64_to_ui64( to_softfloat64(_lcscs_f64_trunc( af )), 0, false );
      }
      float _lcscs_i32_to_f32( int32_t a )  {
         return from_softfloat32(i32_to_f32( a ));
      }
      float _lcscs_i64_to_f32( int64_t a ) {
         return from_softfloat32(i64_to_f32( a ));
      }
      float _lcscs_ui32_to_f32( uint32_t a ) {
         return from_softfloat32(ui32_to_f32( a ));
      }
      float _lcscs_ui64_to_f32( uint64_t a ) {
         return from_softfloat32(ui64_to_f32( a ));
      }
      double _lcscs_i32_to_f64( int32_t a ) {
         return from_softfloat64(i32_to_f64( a ));
      }
      double _lcscs_i64_to_f64( int64_t a ) {
         return from_softfloat64(i64_to_f64( a ));
      }
      double _lcscs_ui32_to_f64( uint32_t a ) {
         return from_softfloat64(ui32_to_f64( a ));
      }
      double _lcscs_ui64_to_f64( uint64_t a ) {
...
class crypto_api : public context_aware_api {
   public:
      ...
      void assert_recover_key( const fc::sha256& digest,
         ...
         scs_ASSERT(s.which() < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_signature_type,
           "Unactivated signature type used during assert_recover_key");
         scs_ASSERT(p.which() < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_key_type,
           "Unactivated key type used when creating assert_recover_key");

         if(context.control.is_producing_block())
            scs_ASSERT(s.variable_size() <= context.control.configured_subjective_signature_length_limit(),
                       sig_variable_size_limit_exception, "signature variable length component size greater than subjective maximum");

         auto check = fc::crypto::public_key( s, digest, false );
         scs_ASSERT( check == p, crypto_api_exception, "Error expected key different than recovered key" );
      }
      ...
      int recover_key( const fc::sha256& digest,
         ...
         if (s.which() >= config::genesis_num_supported_key_types ) {
            scs_ASSERT(publen >= 33, wasm_execution_error,
      ...
      template<class Encoder> auto encode(char* data, uint32_t datalen) {
         Encoder e;
         const size_t bs = lcscs::chain::config::hashing_checktime_block_size;
         while ( datalen > bs ) {
            e.write( data, bs );
            data += bs;
            datalen -= bs;
      ...
      void assert_sha256(array_ptr<char> data, uint32_t datalen, const fc::sha256& hash_val) {
         ...
         scs_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }
...
class permission_api : public context_aware_api {
   public:
      ...
      {
         scs_ASSERT( delay_us <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
      ...
      int64_t get_account_creation_time( account_name account ) {
         ...
         scs_ASSERT( acct != nullptr, action_validate_exception,
...
public:
   ...
   void abort() {
      scs_ASSERT( false, abort_called, "abort() called");
   }

   // Kept as intrinsic rather than implementing on WASM side (using lcscs_assert_message and strlen) because strlen is faster on native side.
   void lcscs_assert( bool condition, null_terminated_ptr msg ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         const size_t sz = strnlen( msg, max_assert_message );
         std::string message( msg, sz );
         scs_THROW( lcscs_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }

   void lcscs_assert_message( bool condition, array_ptr<const char> msg, uint32_t msg_len ) {
      if( BOOST_UNLIKELY( !condition ) ) {
   ...
   void lcscs_assert_code( bool condition, uint64_t error_code ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         if( error_code >= static_cast<uint64_t>(system_error_code::generic_system_error) ) {
            restricted_error_code_exception e( FC_LOG_MESSAGE(
                                                   error,
                                                   "lcscs_assert_code called with reserved error code: ${error_code}",
                                                   ("error_code", error_code)
            ) );
            e.error_code = static_cast<uint64_t>(system_error_code::contract_restricted_error_code);
            throw e;
         } else {
            lcscs_assert_code_exception e( FC_LOG_MESSAGE(
                                             error,
...
#define DB_API_METHOD_WRAPPERS_FLOAT_SECONDARY(IDX, TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const TYPE& secondary ) {\
         scs_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
         return context.IDX.store( scope, table, account_name(payer), id, secondary );\
      }\
      void db_##IDX##_update( int iterator, uint64_t payer, const TYPE& secondary ) {\
         scs_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
...
class memory_api : public context_aware_api {
   public:
      ...
      char* memcpy( array_ptr<char> dest, array_ptr<const char> src, uint32_t length) {
         scs_ASSERT((size_t)(std::abs((ptrdiff_t)dest.value - (ptrdiff_t)src.value)) >= length,
...
class transaction_api : public context_aware_api {
   public:
      ...
      void send_inline( array_ptr<char> data, uint32_t data_len ) {
         ...
         scs_ASSERT( data_len < context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
...
class compiler_builtins : public context_aware_api {
   public:
      ...
      void __divti3(__int128& ret, uint64_t la, uint64_t ha, uint64_t lb, uint64_t hb) {
         ...
         scs_ASSERT(rhs != 0, arithmetic_exception, "divide by zero");
...
/*
 * This api will be removed with fix for `scs #2561`
 */
...
REGISTER_INTRINSICS(context_free_system_api,
   (abort,                void()              )
   (lcscs_assert,         void(int, int)      )
   (lcscs_assert_message, void(int, int, int) )
   (lcscs_assert_code,    void(int, int64_t)  )
   (lcscs_exit,           void(int)           )
);
...
REGISTER_INJECTED_INTRINSICS(softfloat_api,
      (_lcscs_f32_add,       float(float, float)    )
      (_lcscs_f32_sub,       float(float, float)    )
      (_lcscs_f32_mul,       float(float, float)    )
      (_lcscs_f32_div,       float(float, float)    )
      (_lcscs_f32_min,       float(float, float)    )
      (_lcscs_f32_max,       float(float, float)    )
      (_lcscs_f32_copysign,  float(float, float)    )
      (_lcscs_f32_abs,       float(float)           )
      (_lcscs_f32_neg,       float(float)           )
      (_lcscs_f32_sqrt,      float(float)           )
      (_lcscs_f32_ceil,      float(float)           )
      (_lcscs_f32_floor,     float(float)           )
      (_lcscs_f32_trunc,     float(float)           )
      (_lcscs_f32_nearest,   float(float)           )
      (_lcscs_f32_eq,        int(float, float)      )
      (_lcscs_f32_ne,        int(float, float)      )
      (_lcscs_f32_lt,        int(float, float)      )
      (_lcscs_f32_le,        int(float, float)      )
      (_lcscs_f32_gt,        int(float, float)      )
      (_lcscs_f32_ge,        int(float, float)      )
      (_lcscs_f64_add,       double(double, double) )
      (_lcscs_f64_sub,       double(double, double) )
      (_lcscs_f64_mul,       double(double, double) )
      (_lcscs_f64_div,       double(double, double) )
      (_lcscs_f64_min,       double(double, double) )
      (_lcscs_f64_max,       double(double, double) )
      (_lcscs_f64_copysign,  double(double, double) )
      (_lcscs_f64_abs,       double(double)         )
      (_lcscs_f64_neg,       double(double)         )
      (_lcscs_f64_sqrt,      double(double)         )
      (_lcscs_f64_ceil,      double(double)         )
      (_lcscs_f64_floor,     double(double)         )
      (_lcscs_f64_trunc,     double(double)         )
      (_lcscs_f64_nearest,   double(double)         )
      (_lcscs_f64_eq,        int(double, double)    )
      (_lcscs_f64_ne,        int(double, double)    )
      (_lcscs_f64_lt,        int(double, double)    )
      (_lcscs_f64_le,        int(double, double)    )
      (_lcscs_f64_gt,        int(double, double)    )
      (_lcscs_f64_ge,        int(double, double)    )
      (_lcscs_f32_promote,    double(float)         )
      (_lcscs_f64_demote,     float(double)         )
      (_lcscs_f32_trunc_i32s, int(float)            )
      (_lcscs_f64_trunc_i32s, int(double)           )
      (_lcscs_f32_trunc_i32u, int(float)            )
      (_lcscs_f64_trunc_i32u, int(double)           )
      (_lcscs_f32_trunc_i64s, int64_t(float)        )
      (_lcscs_f64_trunc_i64s, int64_t(double)       )
      (_lcscs_f32_trunc_i64u, int64_t(float)        )
      (_lcscs_f64_trunc_i64u, int64_t(double)       )
      (_lcscs_i32_to_f32,     float(int32_t)        )
      (_lcscs_i64_to_f32,     float(int64_t)        )
      (_lcscs_ui32_to_f32,    float(int32_t)        )
      (_lcscs_ui64_to_f32,    float(int64_t)        )
      (_lcscs_i32_to_f64,     double(int32_t)       )
      (_lcscs_i64_to_f64,     double(int64_t)       )
      (_lcscs_ui32_to_f64,    double(int32_t)       )
      (_lcscs_ui64_to_f64,    double(int64_t)       )
);
...
std::istream& operator>>(std::istream& in, wasm_interface::vm_type& runtime) {
   ...
   in >> s;
   if (s == "wabt")
      runtime = lcscs::chain::wasm_interface::vm_type::wabt;
   else if (s == "scs-vm")
      runtime = lcscs::chain::wasm_interface::vm_type::scs_vm;
   else if (s == "scs-vm-jit")
      runtime = lcscs::chain::wasm_interface::vm_type::scs_vm_jit;
   else if (s == "scs-vm-oc")
      runtime = lcscs::chain::wasm_interface::vm_type::scs_vm_oc;
   else
