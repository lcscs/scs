...
#include <lcscs/chain/block_summary_object.hpp>
...
#include <lcscs/chain/contract_table_objects.hpp>
#include <lcscs/chain/generated_transaction_object.hpp>
#include <lcscs/chain/transaction_object.hpp>
#include <lcscs/chain/reversible_block_object.hpp>
#include <lcscs/chain/genesis_intrinsics.hpp>
#include <lcscs/chain/whitelisted_intrinsics.hpp>
#include <lcscs/chain/database_header_object.hpp>

#include <lcscs/chain/protocol_feature_manager.hpp>
#include <lcscs/chain/authorization_manager.hpp>
#include <lcscs/chain/resource_limits.hpp>
#include <lcscs/chain/chain_snapshot.hpp>
#include <lcscs/chain/thread_utils.hpp>
#include <lcscs/chain/platform_timer.hpp>
...
struct controller_impl {
   ...
   void pop_block() {
      ...
      if( !prev ) {
         scs_ASSERT( fork_db.root()->id == head->header.previous, block_validate_exception, "attempt to pop beyond last irreversible block" );
         prev = fork_db.root();
      }
      ...
      if ( read_mode == db_read_mode::SPECULATIVE ) {
         scs_ASSERT( head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
      }
   ...
   inline void set_activation_handler() {
      ...
      scs_ASSERT( res.second, misc_exception, "attempting to set activation handler twice" );
   }
   ...
   :rnh(),
    ...
    wasmif( cfg.wasm_runtime, cfg.scsvmoc_tierup, db, cfg.state_dir, cfg.scsvmoc_config ),
    resource_limits( db ),
    authorization( s, db ),
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( lcscs, lcscs, newaccount );
   SET_APP_HANDLER( lcscs, lcscs, setcode );
   SET_APP_HANDLER( lcscs, lcscs, setabi );
   SET_APP_HANDLER( lcscs, lcscs, updateauth );
   SET_APP_HANDLER( lcscs, lcscs, deleteauth );
   SET_APP_HANDLER( lcscs, lcscs, linkauth );
   SET_APP_HANDLER( lcscs, lcscs, unlinkauth );
/*
   SET_APP_HANDLER( lcscs, lcscs, postrecovery );
   SET_APP_HANDLER( lcscs, lcscs, passrecovery );
   SET_APP_HANDLER( lcscs, lcscs, vetorecovery );
*/

   SET_APP_HANDLER( lcscs, lcscs, canceldelay );
   }
   ...
   void log_irreversible() {
      scs_ASSERT( fork_db.root(), fork_database_exception, "fork database not properly initialized" );
      ...
      if( log_head ) {
         scs_ASSERT( root_id == blog.head_id(), fork_database_exception, "fork database root does not match block log head" );
      } else {
         scs_ASSERT( fork_db.root()->block_num == lib_num, fork_database_exception,
   ...
   void replay(std::function<bool()> shutdown) {
      ...
      if( start_block_num <= blog_head->block_num() ) {
         ...
         if( pending_head->block_num < head->block_num || head->block_num < fork_db.root()->block_num ) {
            ...
            scs_ASSERT( new_root, fork_database_exception, "unexpected error: could not find new LIB in fork database" );
   ...
   void startup(std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot) {
      scs_ASSERT( snapshot, snapshot_exception, "No snapshot reader provided" );
      ...
      try {
         ...
         if( blog.head() ) {
            ...
            scs_ASSERT( lib_num > 0, snapshot_exception,
   ...
   void startup(std::function<bool()> shutdown, const genesis_state& genesis) {
      scs_ASSERT( db.revision() < 1, database_exception, "This version of controller::startup only works with a fresh state database." );
      const auto& genesis_chain_id = genesis.compute_chain_id();
      scs_ASSERT( genesis_chain_id == chain_id, chain_id_type_exception,
      ...
      if( blog.head() ) {
         scs_ASSERT( blog.first_block_num() == 1, block_log_exception,
   ...
   void startup(std::function<bool()> shutdown) {
      scs_ASSERT( db.revision() >= 1, database_exception, "This version of controller::startup does not work with a fresh state database." );
      scs_ASSERT( fork_db.head(), fork_database_exception, "No existing fork database despite existing chain state. Replay required." );

      uint32_t lib_num = fork_db.root()->block_num;
      auto first_block_num = blog.first_block_num();
      if( blog.head() ) {
         scs_ASSERT( first_block_num <= lib_num && lib_num <= blog.head()->block_num(),
                     block_log_exception,
   ...
   static auto validate_db_version( const chainbase::database& db ) {
      ...
      scs_ASSERT(header_idx.begin() != header_idx.end(), bad_database_version_exception,
   ...
   void init(std::function<bool()> shutdown) {
...

      {
         ...
         scs_ASSERT( state_chain_id == chain_id, chain_id_type_exception,
      ...
      scs_ASSERT( db.revision() >= head->block_num, fork_database_exception,
      ...
      if( read_mode == db_read_mode::IRREVERSIBLE ) {
         ...
         scs_ASSERT( itr == rbi.end() || itr->blocknum == lib_num + 1, reversible_blocks_exception,
         ...
         scs_ASSERT( head->block_num <= last_block_num, reversible_blocks_exception,
         ...
         if( ritr != rbi.rend()
            ...
            scs_ASSERT( rbitr != rbi.end(), reversible_blocks_exception, "pending head block not found in reversible blocks");
            auto rev_id = rbitr->get_block_id();
            scs_ASSERT( rev_id == pending_head->id,
   ...
   void read_from_snapshot( const snapshot_reader_ptr& snapshot, uint32_t blog_start, uint32_t blog_end ) {
      ...
      { /// load and upgrade the block header state
         ...
         scs_ASSERT( blog_start <= (snapshot_head_block + 1) && snapshot_head_block <= blog_end,
                     block_log_exception,
      ...
      controller_index_set::walk_indices([this, &snapshot, &header]( auto utils ){
         ...
         if (std::is_same<value_t, global_property_object>::value) {
            ...
            if (std::clamp(header.version, v2::minimum_version, v2::maximum_version) == header.version ) {
               ...
               scs_ASSERT( genesis, snapshot_exception,
      ...
      scs_ASSERT( gpo.chain_id == chain_id, chain_id_type_exception,
   ...
   void create_native_account( const fc::time_point& initial_timestamp, account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            // The initial lcscs ABI value affects consensus; see  https://github.com/lcscs/scs/issues/7794
            // TODO: This doesn't charge RAM; a fix requires a consensus upgrade.
            a.abi.resize(sizeof(lcscs_abi_bin));
            memcpy(a.abi.data(), lcscs_abi_bin, sizeof(lcscs_abi_bin));
         }
      });
   ...
   transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false ) {
      ...
      scs_ASSERT( itr != idx.end(), unknown_transaction_exception, "unknown transaction" );
   ...
   { try {
      ...
      scs_ASSERT( !validating || explicit_billed_cpu_time, transaction_exception, "validating requires explicit billing" );
      ...
      scs_ASSERT( gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
      ...
      if ( !subjective ) {
         ...
         if( !validating ) {
            ...
            scs_ASSERT( !explicit_billed_cpu_time || (cpu_time_to_bill_us == limited_cpu_time_to_bill_us),
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      scs_ASSERT( net_usage_words*8 == net_usage, transaction_exception, "net_usage is not divisible by 8" );
   ...
   {
      scs_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
      ...
      try {
         ...
         if( !explicit_billed_cpu_time ) {
            fc::microseconds already_consumed_time( scs_PERCENT(sig_cpu_usage.count(), conf.sig_cpu_bill_pct) );
   ...
   {
      scs_ASSERT( !pending, block_validate_exception, "pending block already exists" );
      ...
      if (!self.skip_db_sessions(s)) {
         scs_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
      ...
      if ( read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete )
      {
         ...
         if( new_protocol_feature_activations.size() > 0 ) {
            ...
            for( const auto& feature_digest : new_protocol_feature_activations ) {
               ...
               if( res.second ) {
                  ...
                  scs_ASSERT( !f.preactivation_required, protocol_feature_exception,
                              "attempted to activate protocol feature without prior required preactivation: ${digest}",
                              ("digest", feature_digest)
                  );
               } else {
                  scs_ASSERT( !res.first->second, block_validate_exception,
         ...
         scs_ASSERT( handled_all_preactivated_features, block_validate_exception,
         ...
         )
         {
            ...
            scs_ASSERT( gpo.proposed_schedule.version == pbhs.active_schedule_version + 1,
   ...
   void finalize_block()
   {
      scs_ASSERT( pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
      scs_ASSERT( pending->_block_stage.contains<building_block>(), block_validate_exception, "already called finalize_block");
      ...
      uint64_t CPU_TARGET = scs_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {scs_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, config::maximum_elastic_resource_multiplier, {99, 100}, {1000, 999}}
      );
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      try {
         scs_ASSERT( pending->_block_stage.contains<completed_block>(), block_validate_exception,
         ...
         if( add_to_fork_db ) {
            ...
            head = fork_db.head();
            scs_ASSERT( bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
      ...
      for( auto itr = new_protocol_features.begin(); itr != new_protocol_features.end(); ++itr ) {
         ...
         switch( status ) {
            case protocol_feature_set::recognized_t::unrecognized:
               scs_THROW( protocol_feature_exception,
            ...
            break;
            default:
               scs_THROW( protocol_feature_exception, "unexpected recognized_t status" );
            break;
         }

         scs_ASSERT( currently_activated_protocol_features.find( f ) == currently_activated_protocol_features.end(),
         ...
         scs_ASSERT( pfs.validate_dependencies( f, dependency_checker ), protocol_feature_exception,
...
#define scs_REPORT(DESC,A,B) \
   if( A != B ) { \
      elog("${desc}: ${bv} != ${abv}", ("desc", DESC)("bv", A)("abv", B)); \
   }

      scs_REPORT( "timestamp", b.timestamp, ab.timestamp )
      scs_REPORT( "producer", b.producer, ab.producer )
      scs_REPORT( "confirmed", b.confirmed, ab.confirmed )
      scs_REPORT( "previous", b.previous, ab.previous )
      scs_REPORT( "transaction_mroot", b.transaction_mroot, ab.transaction_mroot )
      scs_REPORT( "action_mroot", b.action_mroot, ab.action_mroot )
      scs_REPORT( "schedule_version", b.schedule_version, ab.schedule_version )
      scs_REPORT( "new_producers", b.new_producers, ab.new_producers )
      scs_REPORT( "header_extensions", b.header_extensions, ab.header_extensions )
...
#undef scs_REPORT
      ...
      try {
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               ...
               scs_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
         ...
         if( producer_block_id != ab._id ) {
            ...
            scs_ASSERT( producer_block_id == ab._id, block_validate_exception, "Block ID does not match",
   ...
   std::future<block_state_ptr> create_block_state_future( const signed_block_ptr& b ) {
      scs_ASSERT( b, block_validate_exception, "null block" );

      auto id = b->id();

      // no reason for a block_state if fork_db already knows about block
      auto existing = fork_db.get_block( id );
      scs_ASSERT( !existing, fork_database_exception, "we already know about this block: ${id}", ("id", id) );

      auto prev = fork_db.get_block_header( b->previous );
      scs_ASSERT( prev, unlinkable_block_exception,
      ...
      return async_thread_pool( thread_pool.get_executor(), [b, prev, control=this]() {
         ...
         scs_ASSERT( b->transaction_mroot == trx_mroot, block_validate_exception,
                    ...
                    const forked_branch_callback& forked_branch_cb, const trx_meta_cache_lookup& trx_lookup )
   {
      ...
      scs_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");
   ...
   void replay_push_block( const signed_block_ptr& b, controller::block_status s ) {
      ...
      try {
         scs_ASSERT( b, block_validate_exception, "trying to push empty block" );
         scs_ASSERT( (s == controller::block_status::irreversible || s == controller::block_status::validated),
         ...
         if( s == controller::block_status::irreversible ) {
            ...
            scs_ASSERT( read_mode != db_read_mode::IRREVERSIBLE, block_validate_exception,
                            ...
                            const forked_branch_callback& forked_branch_cb, const trx_meta_cache_lookup& trx_lookup )
   {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         if( branches.second.size() > 0 ) {
            ...
            scs_ASSERT( self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      ...
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         scs_ASSERT( is_subset,  actor_whitelist_exception,
         ...
         scs_ASSERT( !intersects, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      if( conf.contract_whitelist.size() > 0 ) {
         scs_ASSERT( conf.contract_whitelist.find( code ) != conf.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( conf.contract_blacklist.size() > 0 ) {
         scs_ASSERT( conf.contract_blacklist.find( code ) == conf.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( conf.action_blacklist.size() > 0 ) {
         scs_ASSERT( conf.action_blacklist.find( std::make_pair(code, action) ) == conf.action_blacklist.end(),
   ...
   void check_key_list( const public_key_type& key )const {
      if( conf.key_blacklist.size() > 0 ) {
         scs_ASSERT( conf.key_blacklist.find( key ) == conf.key_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      scs_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
void controller::preactivate_feature( const digest_type& feature_digest ) {
   ...
   switch( status ) {
      case protocol_feature_set::recognized_t::unrecognized:
         if( is_producing_block() ) {
            scs_THROW( subjective_block_production_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         } else {
            scs_THROW( protocol_feature_bad_block_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         }
      break;
      case protocol_feature_set::recognized_t::disabled:
      ...
      default:
         if( is_producing_block() ) {
            scs_THROW( subjective_block_production_exception, "unexpected recognized_t status" );
         } else {
            scs_THROW( protocol_feature_bad_block_exception, "unexpected recognized_t status" );
         }
      break;
   }
   ...
   // Even if they don't recognize the feature, the producer could change their nodscs code to treat it like an
   ...
   scs_ASSERT( !is_protocol_feature_activated( feature_digest ),
               protocol_feature_exception,
   ...
   scs_ASSERT( std::find( pso.preactivated_protocol_features.begin(),
   ...
   scs_ASSERT( pfs.validate_dependencies( feature_digest, dependency_checker ),
               protocol_feature_exception,
...
void controller::start_block( block_timestamp_type when, uint16_t confirm_block_count )
{
   ...
   scs_ASSERT( !my->pending, block_validate_exception, "pending block already exists" );
...
transaction_trace_ptr controller::push_transaction( const transaction_metadata_ptr& trx, fc::time_point deadline,
   ...
   validate_db_available_size();
   scs_ASSERT( get_read_mode() != db_read_mode::IRREVERSIBLE, transaction_type_exception, "push transaction not allowed in irreversible mode" );
   scs_ASSERT( trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed" );
...
transaction_trace_ptr controller::push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline,
                                                              uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time )
{
   scs_ASSERT( get_read_mode() != db_read_mode::IRREVERSIBLE, transaction_type_exception, "push scheduled transaction not allowed in irreversible mode" );
   validate_db_available_size();
...
void controller::set_action_blacklist( const flat_set< pair<account_name, action_name> >& new_action_blacklist ) {
   for (auto& act: new_action_blacklist) {
      scs_ASSERT(act.first != account_name(), name_type_exception, "Action blacklist - contract name should not be empty");
      scs_ASSERT(act.second != action_name(), action_type_exception, "Action blacklist - action name should not be empty");
   }
...
time_point controller::pending_block_time()const {
   scs_ASSERT( my->pending, block_validate_exception, "no pending block" );
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   scs_ASSERT( BOOST_LIKELY( id != block_id_type() ), unknown_block_exception,
...
void controller::write_snapshot( const snapshot_writer_ptr& snapshot ) const {
   scs_ASSERT( !my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block" );
...
void controller::validate_expiration( const transaction& trx )const { try {
   ...
   scs_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   scs_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
...
void controller::validate_db_available_size() const {
   ...
   scs_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::validate_reversible_available_size() const {
   ...
   scs_ASSERT(free >= guard, reversible_guard_exception, "reversible free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::set_greylist_limit( uint32_t limit ) {
   scs_ASSERT( 0 < limit && limit <= chain::config::maximum_elastic_resource_multiplier,
               misc_exception,
