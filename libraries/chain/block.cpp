...
namespace lcscs { namespace chain {
   void additional_block_signatures_extension::reflector_init() {
      ...
      scs_ASSERT( signatures.size() > 0, ill_formed_additional_block_signatures_extension,
      ...
      for( const auto& s : signatures ) {
         ...
         scs_ASSERT( res.second, ill_formed_additional_block_signatures_extension,
   ...
   flat_multimap<uint16_t, block_extension> signed_block::validate_and_extract_extensions()const {
      ...
      for( size_t i = 0; i < block_extensions.size(); ++i ) {
         ...
         scs_ASSERT( id >= id_type_lower_bound, invalid_block_extension,
         ...
         scs_ASSERT( match, invalid_block_extension,
                     "Block extension with id type ${id} is not supported",
                     ("id", id)
         );

         if( match->enforce_unique ) {
            scs_ASSERT( i == 0 || id > id_type_lower_bound, invalid_block_header_extension,
