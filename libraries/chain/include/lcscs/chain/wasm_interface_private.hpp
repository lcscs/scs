...
#include <lcscs/chain/webassembly/wavm.hpp>
#include <lcscs/chain/webassembly/wabt.hpp>
#ifdef lcscs_scs_VM_OC_RUNTIME_ENABLED
#include <lcscs/chain/webassembly/scs-vm-oc.hpp>
#else
#define _REGISTER_scsVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif
...
#include <lcscs/chain/webassembly/scs-vm.hpp>
...
using namespace fc;
using namespace lcscs::chain::webassembly;
using namespace IR;
...
namespace lcscs { namespace chain {
   ...
   namespace scsvmoc { struct config; }
   ...
   struct wasm_interface_impl {
...
#ifdef lcscs_scs_VM_OC_RUNTIME_ENABLED
      struct scsvmoc_tier {
         scsvmoc_tier(const boost::filesystem::path& d, const scsvmoc::config& c, const chainbase::database& db) : cc(d, c, db), exec(cc) {}
         scsvmoc::code_cache_async cc;
         scsvmoc::executor exec;
         scsvmoc::memory mem;
      };
#endif

      wasm_interface_impl(wasm_interface::vm_type vm, bool scsvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const scsvmoc::config& scsvmoc_config) : db(d), wasm_runtime_time(vm) {
         ...
         if(vm == wasm_interface::vm_type::scs_vm)
            runtime_interface = std::make_unique<webassembly::scs_vm_runtime::scs_vm_runtime<lcscs::vm::interpreter>>();
#endif
#ifdef lcscs_scs_VM_JIT_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::scs_vm_jit)
            runtime_interface = std::make_unique<webassembly::scs_vm_runtime::scs_vm_runtime<lcscs::vm::jit>>();
#endif
#ifdef lcscs_scs_VM_OC_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::scs_vm_oc)
            runtime_interface = std::make_unique<webassembly::scsvmoc::scsvmoc_runtime>(data_dir, scsvmoc_config, d);
#endif
         if(!runtime_interface)
            scs_THROW(wasm_exception, "${r} wasm runtime not supported on this platform and/or configuration", ("r", vm));

#ifdef lcscs_scs_VM_OC_RUNTIME_ENABLED
         if(scsvmoc_tierup) {
            scs_ASSERT(vm != wasm_interface::vm_type::scs_vm_oc, wasm_exception, "You can't use scs VM OC as the base runtime when tier up is activated");
            scsvmoc.emplace(data_dir, scsvmoc_config, d);
         }
#endif
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            scs_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            scs_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#ifdef lcscs_scs_VM_OC_RUNTIME_ENABLED
         if(scsvmoc) for(auto it = first_it; it != last_it; it++)
            scsvmoc->cc.free_code(it->code_hash, it->vm_version);
#endif
         ...
         if(!it->module) {
            ...
            if (runtime_interface->inject_module(module)) {
               try {
                  ...
                  scs_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
               } catch (const IR::ValidationException& e) {
                  scs_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
...
#ifdef lcscs_scs_VM_OC_RUNTIME_ENABLED
      fc::optional<scsvmoc_tier> scsvmoc;
#endif
   };
...
#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   ...
   _REGISTER_scs_VM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)       \
   _REGISTER_scsVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, lcscs_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
