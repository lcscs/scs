...
struct abi_serializer {
   ...
   static yield_function_t create_yield_function(const fc::microseconds& max_serialization_time) {
      ...
      return [max_serialization_time, deadline](size_t recursion_depth) {
         scs_ASSERT( recursion_depth < max_recursion_depth, abi_recursion_depth_exception,
                     "recursive definition, max_recursion_depth ${r} ", ("r", max_recursion_depth) );

         scs_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception,
...
namespace impl {
   ...
   struct abi_from_variant {
      ...
      static void extract( const variant& v, action& act, Resolver resolver, abi_traverse_context& ctx )
      {
         auto h = ctx.enter_scope();
         const variant_object& vo = v.get_object();
         scs_ASSERT(vo.contains("account"), packed_transaction_type_exception, "Missing account");
         scs_ASSERT(vo.contains("name"), packed_transaction_type_exception, "Missing name");
         ...
         scs_ASSERT(valid_empty_data || !act.data.empty(), packed_transaction_type_exception,
      ...
      static void extract( const variant& v, packed_transaction& ptrx, Resolver resolver, abi_traverse_context& ctx )
      {
         auto h = ctx.enter_scope();
         const variant_object& vo = v.get_object();
         scs_ASSERT(vo.contains("signatures"), packed_transaction_type_exception, "Missing signatures");
         scs_ASSERT(vo.contains("compression"), packed_transaction_type_exception, "Missing compression");
         ...
         if( vo.contains("packed_trx") && vo["packed_trx"].is_string() && !vo["packed_trx"].as_string().empty() ) {
            ...
            scs_ASSERT(vo.contains("transaction"), packed_transaction_type_exception, "Missing transaction");
            if( use_packed_cfd ) {
               transaction trx;
