...
namespace lcscs { namespace chain {
      ...
      class database_header_object : public chainbase::object<database_header_object_type, database_header_object>
      {
         ...
         void validate() const {
            scs_ASSERT(std::clamp(version, minimum_version, current_version) == version, bad_database_version_exception,
...
CHAINBASE_SET_INDEX_TYPE(lcscs::chain::database_header_object, lcscs::chain::database_header_multi_index)

FC_REFLECT( lcscs::chain::database_header_object, (version) )
