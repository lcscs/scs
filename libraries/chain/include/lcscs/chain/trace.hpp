...
#include <lcscs/chain/action_receipt.hpp>
...
namespace lcscs { namespace chain {
   ...
   inline bool is_onblock( const transaction_trace& tt ) {
      ...
      if (act.account != lcscs::chain::config::system_account_name || act.name != N(onblock) ||
          act.authorization.size() != 1)
         return false;
      const auto& auth = act.authorization[0];
      return auth.actor == lcscs::chain::config::system_account_name &&
             auth.permission == lcscs::chain::config::active_name;
   }

} }  /// namespace lcscs::chain

FC_REFLECT( lcscs::chain::account_delta,
            (account)(delta) )

FC_REFLECT( lcscs::chain::action_trace,
...
FC_REFLECT( lcscs::chain::transaction_trace, (id)(block_num)(block_time)(producer_block_id)
