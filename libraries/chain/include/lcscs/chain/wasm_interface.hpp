      ...
      public:
         ...
         { try {
            ...
            if( whitelisted_intrinsics != nullptr ) {
               ...
               scs_ASSERT( mod_name == "env", wasm_exception,
            ...
            scs_THROW( wasm_exception, "${module}.${export} unresolveable",
                      ("module",mod_name)("export",export_name) );
            return false;
   ...
   class wasm_interface {
      public:
         ...
         static std::string vm_type_string(vm_type vmtype) {
             ...
             case vm_type::scs_vm:
                return "scs-vm";
             case vm_type::scs_vm_jit:
                return "scs-vm-jit";
             case vm_type::scs_vm_oc:
                return "scs-vm-oc";
             default:
                return "wabt";
             }
         ...
         wasm_interface(vm_type vm, bool scsvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const scsvmoc::config& scsvmoc_config);
         ~wasm_interface();

         //call before dtor to skip what can be minutes of dtor overhead with some runtimes; can cause leaks
         void indicate_shutting_down();

         //validates code -- does a WASM validation pass and checks the wasm against lcscs specific constraints
      ...
      private:
         ...
         friend class lcscs::chain::webassembly::common::intrinsics_accessor;
   };
...
FC_REFLECT_ENUM( lcscs::chain::wasm_interface::vm_type, (wabt)(scs_vm)(scs_vm_jit)(scs_vm_oc) )
