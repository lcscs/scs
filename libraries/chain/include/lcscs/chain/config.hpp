...
namespace lcscs { namespace chain { namespace config {
...
const static name system_account_name    { N(lcscs) };
const static name null_account_name      { N(scs.null) };
const static name producers_account_name { N(scs.prods) };

// Active permission of producers account requires greater than 2/3 of the producers to authorize
const static name majority_producers_permission_name { N(scs.major) }; // greater than 1/2 of producers needed to authorize
const static name minority_producers_permission_name { N(scs.minor) }; // greater than 1/3 of producers needed to authorize0

const static name lcscs_auth_scope       { N(scs.auth) };
const static name lcscs_all_scope        { N(scs.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name lcscs_any_name  { N(scs.any) };
const static name lcscs_code_name { N(scs.code) };
...
const static lcscs::chain::wasm_interface::vm_type default_wasm_runtime = lcscs::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace lcscs::chain::config

constexpr uint64_t scs_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / lcscs::chain::config::percent_100;
}

template<typename Number>
Number scs_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + lcscs::chain::config::percent_100 - lcscs::chain::config::percent_1)  / lcscs::chain::config::percent_100;
}
