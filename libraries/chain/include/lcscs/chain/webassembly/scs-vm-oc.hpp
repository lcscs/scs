...
using namespace lcscs::chain::scsvmoc;

class scsvmoc_instantiated_module;

class scsvmoc_runtime : public lcscs::chain::wasm_runtime_interface {
   public:
      scsvmoc_runtime(const boost::filesystem::path data_dir, const scsvmoc::config& scsvmoc_config, const chainbase::database& db);
      ~scsvmoc_runtime();
      ...
      friend scsvmoc_instantiated_module;
      scsvmoc::code_cache_sync cc;
...
inline array_ptr<T> array_ptr_impl (size_t ptr, size_t length)
{
   ...
   asm volatile("cmp %%gs:%c[firstInvalidMemory], %[End]\n"
                "jle 1f\n"
                "mov %%gs:%c[maximumlcscsWasmMemory], %[Ptr]\n"      //always invalid address
                "1:\n"
                ...
                : [linearMemoryStart] "i" (cb_full_linear_memory_start_segment_offset),
                  ...
                  [maximumlcscsWasmMemory] "i" (wasm_constraints::maximum_linear_memory)
                : "cc"
               );
   ...
   static native_to_wasm_t<Ret> invoke(Translated... translated) {
      try {
         if constexpr(!is_injected) {
            ...
            constexpr int depth_assertion_intrinsic_offset = OFFSET_OF_FIRST_INTRINSIC - (int)boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("scsvmoc_internal.depth_assert"))).value()*8;
         ...
         *reinterpret_cast<std::exception_ptr*>(scs_vm_oc_get_exception_ptr()) = std::current_exception();
      }
...
struct intrinsic_invoker_impl<is_injected, Ret, std::tuple<array_ptr<T>, uint32_t, Inputs...>, std::tuple<Translated...>> {
   ...
   static auto translate_one(Inputs... rest, Translated... translated, I32 ptr, I32 size) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      ...
      if ( reinterpret_cast<uintptr_t>(base) % alignof(T) != 0 ) {
         std::vector<std::byte>& copy = reinterpret_cast<std::list<std::vector<std::byte>>*>(scs_vm_oc_get_bounce_buffer_list())->emplace_back(length > 0 ? length*sizeof(T) : 1);
         T* copy_ptr = (T*)&copy[0];
...
struct intrinsic_invoker_impl<is_injected, Ret, std::tuple<T &, Inputs...>, std::tuple<Translated...>> {
   ...
   template<then_type Then, typename U=T>
   static auto translate_one(Inputs... rest, Translated... translated, I32 ptr) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      scs_ASSERT((U32)ptr != 0, wasm_exception, "references cannot be created for null pointers");
   ...
   template<then_type Then, typename U=T>
   static auto translate_one(Inputs... rest, Translated... translated, I32 ptr) -> std::enable_if_t<!std::is_const<U>::value, Ret> {
      scs_ASSERT((U32)ptr != 0, wasm_exception, "reference cannot be created for null pointers");
      T &base = *array_ptr_impl<T>((uint32_t)ptr, 1);
...
#define _REGISTER_scsVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static lcscs::chain::scsvmoc::intrinsic _INTRINSIC_NAME(__intrinsic_fn, __COUNTER__) scsVMOC_INTRINSIC_INIT_PRIORITY (\
      MOD "." NAME,\
      lcscs::chain::webassembly::scsvmoc::wasm_function_type_provider<WASM_SIG>::type(),\
      (void *)lcscs::chain::webassembly::scsvmoc::intrinsic_function_invoker_wrapper<std::string_view(MOD) != "env", WASM_SIG, SIG>::type::fn<&CLS::METHOD>(),\
      ::boost::hana::index_if(lcscs::chain::scsvmoc::intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING(MOD "." NAME))).value()\
   );\


} } } }// lcscs::chain::webassembly::scsvmoc
