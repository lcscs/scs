...
#include <lcscs/chain/webassembly/scs-vm-oc/gs_seg_helpers.h>
...
class memory {
      static constexpr uint64_t wasm_memory_size                  = lcscs::chain::wasm_constraints::maximum_linear_memory;
      ...
      static constexpr uint64_t mutable_global_size               = 8u  * lcscs::chain::wasm_constraints::maximum_mutable_globals/4u;
      static constexpr uint64_t table_size                        = 16u * lcscs::chain::wasm_constraints::maximum_table_elements;
      static constexpr size_t   wcb_allowance                     = 512u;
      static_assert(sizeof(control_block) <= wcb_allowance, "scs VM OC memory doesn't set aside enough memory for control block");
   ...
   public:
      ...
      static_assert(-cb_offset == scs_VM_OC_CONTROL_BLOCK_OFFSET, "scs VM OC control block offset has slid out of place somehow");
      static_assert(stride == scs_VM_OC_MEMORY_STRIDE, "scs VM OC memory stride has slid out of place somehow");
...
#define OFFSET_OF_CONTROL_BLOCK_MEMBER(M) (-(int)lcscs::chain::scsvmoc::memory::cb_offset + (int)offsetof(lcscs::chain::scsvmoc::control_block, M))
#define OFFSET_OF_FIRST_INTRINSIC ((int)-lcscs::chain::scsvmoc::memory::first_intrinsic_offset)