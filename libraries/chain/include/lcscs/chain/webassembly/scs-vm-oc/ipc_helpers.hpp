...
std::tuple<bool, scsvmoc_message, std::vector<wrapped_fd>> read_message_with_fds(boost::asio::local::datagram_protocol::socket& s);
std::tuple<bool, scsvmoc_message, std::vector<wrapped_fd>> read_message_with_fds(int fd);
bool write_message_with_fds(boost::asio::local::datagram_protocol::socket& s, const scsvmoc_message& message, const std::vector<wrapped_fd>& fds = std::vector<wrapped_fd>());
bool write_message_with_fds(int fd_to_send_to, const scsvmoc_message& message, const std::vector<wrapped_fd>& fds = std::vector<wrapped_fd>());

template<typename T>
wrapped_fd memfd_for_bytearray(const T& bytes) {
   int fd = syscall(SYS_memfd_create, "scsvmoc_code", MFD_CLOEXEC);
