...
namespace fc {
  class variant;
  void to_variant(const lcscs::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, lcscs::chain::name& check);
} // fc
...
namespace lcscs::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, lcscs::chain::name& check);
...
#define N(X) lcscs::chain::string_to_name(#X)

} // lcscs::chain

namespace std {
   template<> struct hash<lcscs::chain::name> : private hash<uint64_t> {
      typedef lcscs::chain::name argument_type;
...
FC_REFLECT( lcscs::chain::name, (value) )
