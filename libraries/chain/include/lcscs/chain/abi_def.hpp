...
abi_def lcscs_contract_abi(const abi_def& lcscs_system_abi);
vector<type_def> common_type_defs();

extern unsigned char lcscs_abi_bin[2132];
...
datastream<ST>& operator << (datastream<ST>& s, const lcscs::chain::may_not_exist<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
datastream<ST>& operator >> (datastream<ST>& s, lcscs::chain::may_not_exist<T>& v) {
   if (s.remaining())
...
template<typename T>
void to_variant(const lcscs::chain::may_not_exist<T>& e, fc::variant& v) {
   to_variant( e.value, v);
}

template<typename T>
void from_variant(const fc::variant& v, lcscs::chain::may_not_exist<T>& e) {
   from_variant( v, e.value );
}

} // namespace fc

FC_REFLECT( lcscs::chain::type_def                         , (new_type_name)(type) )
FC_REFLECT( lcscs::chain::field_def                        , (name)(type) )
FC_REFLECT( lcscs::chain::struct_def                       , (name)(base)(fields) )
FC_REFLECT( lcscs::chain::action_def                       , (name)(type)(ricardian_contract) )
FC_REFLECT( lcscs::chain::table_def                        , (name)(index_type)(key_names)(key_types)(type) )
FC_REFLECT( lcscs::chain::clause_pair                      , (id)(body) )
FC_REFLECT( lcscs::chain::error_message                    , (error_code)(error_msg) )
FC_REFLECT( lcscs::chain::variant_def                      , (name)(types) )
FC_REFLECT( lcscs::chain::abi_def                          , (version)(types)(structs)(actions)(tables)
