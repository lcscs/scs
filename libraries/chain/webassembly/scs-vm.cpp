...
namespace wasm_constraints = lcscs::chain::wasm_constraints;
...
class scs_vm_instantiated_module : public wasm_instantiated_module_interface {
      using backend_t = backend<apply_context, Impl>;
   public:
      
      scs_vm_instantiated_module(scs_vm_runtime<Impl>* runtime, std::unique_ptr<backend_t> mod) :
         _runtime(runtime),
      ...
      void apply(apply_context& context) override {
         ...
         } catch(lcscs::vm::timeout_exception&) {
            context.trx_context.checktime();
         } catch(lcscs::vm::wasm_memory_exception& e) {
            FC_THROW_EXCEPTION(wasm_execution_error, "access violation");
         } catch(lcscs::vm::exception& e) {
   ...
   private:
      scs_vm_runtime<Impl>*            _runtime;
      std::unique_ptr<backend_t> _instantiated_module;
};

template<typename Impl>
scs_vm_runtime<Impl>::scs_vm_runtime() {}

template<typename Impl>
void scs_vm_runtime<Impl>::immediately_exit_currently_running_module() {
   throw wasm_exit{};
}

template<typename Impl>
bool scs_vm_runtime<Impl>::inject_module(IR::Module& module) {
   return false;
}

template<typename Impl>
std::unique_ptr<wasm_instantiated_module_interface> scs_vm_runtime<Impl>::instantiate_module(const char* code_bytes, size_t code_size, std::vector<uint8_t>,
   ...
   try {
      ...
      return std::make_unique<scs_vm_instantiated_module<Impl>>(this, std::move(bkend));
   } catch(lcscs::vm::exception& e) {
      FC_THROW_EXCEPTION(wasm_execution_error, "Error building scs-vm interp: ${e}", ("e", e.what()));
   }
}

template class scs_vm_runtime<lcscs::vm::interpreter>;
template class scs_vm_runtime<lcscs::vm::jit>;
