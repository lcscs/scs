...
#include <lcscs/chain/webassembly/scs-vm-oc/code_cache.hpp>
#include <lcscs/chain/webassembly/scs-vm-oc/config.hpp>
#include <lcscs/chain/webassembly/common.hpp>
#include <lcscs/chain/webassembly/scs-vm-oc/memory.hpp>
#include <lcscs/chain/webassembly/scs-vm-oc/scs-vm-oc.hpp>
#include <lcscs/chain/webassembly/scs-vm-oc/intrinsic.hpp>
#include <lcscs/chain/webassembly/scs-vm-oc/compile_monitor.hpp>
...
static constexpr uint64_t header_id = 0x32434f4d56534f45ULL; //"scsVMOC2" little endian
...
code_cache_async::code_cache_async(const bfs::path data_dir, const scsvmoc::config& scsvmoc_config, const chainbase::database& db) :
   code_cache_base(data_dir, scsvmoc_config, db),
   _result_queue(scsvmoc_config.threads * 2),
   _threads(scsvmoc_config.threads)
{
   FC_ASSERT(_threads, "scs VM OC requires at least 1 compile thread");
      ...
      if(_outstanding_compiles_and_poison[result.code] == false) {
         result.result.visit(overloaded {
            ...
            [&](const compilation_result_unknownfailure&) {
               wlog("code ${c} failed to tier-up with scs VM OC", ("c", result.code.code_id));
   ...
   if(_outstanding_compiles_and_poison.size()) {
      ...
      while(count_processed && _queued_compiles.size()) {
         ...
         if(codeobject) {
            ...
            FC_ASSERT(write_message_with_fds(_compile_monitor_write_socket, compile_wasm_message{ *nextup }, fds_to_pass), "scs VM failed to communicate to OOP manager");
            --count_processed;
         }
...
code_cache_sync::~code_cache_sync() {
   ...
   auto [success, message, fds] = read_message_with_fds(_compile_monitor_read_socket);
   if(success)
      elog("unexpected response from scs VM OC compile monitor during shutdown");
}
...
const code_descriptor* const code_cache_sync::get_descriptor_for_code_sync(const digest_type& code_id, const uint8_t& vm_version) {
   ...
   auto [success, message, fds] = read_message_with_fds(_compile_monitor_read_socket);
   scs_ASSERT(success, wasm_execution_error, "failed to read response from monitor process");
   scs_ASSERT(message.contains<wasm_compilation_result_message>(), wasm_execution_error, "unexpected response from monitor process");

   wasm_compilation_result_message result = message.get<wasm_compilation_result_message>();
   scs_ASSERT(result.result.contains<code_descriptor>(), wasm_execution_error, "failed to compile wasm");
...
code_cache_base::code_cache_base(const boost::filesystem::path data_dir, const scsvmoc::config& scsvmoc_config, const chainbase::database& db) :
   _db(db),
   _cache_file_path(data_dir/"code_cache.bin")
{
   ...
   if(!bfs::exists(_cache_file_path)) {
      scs_ASSERT(scsvmoc_config.cache_size >= allocator_t::get_min_size(total_header_size), database_exception, "configured code cache size is too small");
      std::ofstream ofs(_cache_file_path.generic_string(), std::ofstream::trunc);
      scs_ASSERT(ofs.good(), database_exception, "unable to create scs VM Optimized Compiler code cache");
      bfs::resize_file(_cache_file_path, scsvmoc_config.cache_size);
      ...
      new (creation_region.get_address()) allocator_t(scsvmoc_config.cache_size, total_header_size);
   ...
   code_cache_header cache_header;
   {
      ...
      scs_ASSERT(!hs.fail(), bad_database_version_exception, "failed to read code cache header");
      memcpy((char*)&cache_header, header_buff + header_offset, sizeof(cache_header));
   }

   scs_ASSERT(cache_header.id == header_id, bad_database_version_exception, "existing scs VM OC code cache not compatible with this version");
   scs_ASSERT(!cache_header.dirty, database_exception, "code cache is dirty");

   set_on_disk_region_dirty(true);

   auto existing_file_size = bfs::file_size(_cache_file_path);
   if(scsvmoc_config.cache_size > existing_file_size) {
      bfs::resize_file(_cache_file_path, scsvmoc_config.cache_size);
      ...
      resize_allocator->grow(scsvmoc_config.cache_size - existing_file_size);
   }

   _cache_fd = ::open(_cache_file_path.generic_string().c_str(), O_RDWR | O_CLOEXEC);
   scs_ASSERT(_cache_fd >= 0, database_exception, "failure to open code cache");

   //load up the previous cache index
   char* code_mapping = (char*)mmap(nullptr, scsvmoc_config.cache_size, PROT_READ|PROT_WRITE, MAP_SHARED, _cache_fd, 0);
   scs_ASSERT(code_mapping != MAP_FAILED, database_exception, "failure to mmap code cache");

   allocator_t* allocator = reinterpret_cast<allocator_t*>(code_mapping);

   if(cache_header.serialized_descriptor_index) {
      fc::datastream<const char*> ds(code_mapping + cache_header.serialized_descriptor_index, scsvmoc_config.cache_size - cache_header.serialized_descriptor_index);
      ...
      ilog("scs VM Optimized Compiler code cache loaded with ${c} entries; ${f} of ${t} bytes free", ("c", number_entries)("f", allocator->get_free_memory())("t", allocator->get_size()));
   }
   munmap(code_mapping, scsvmoc_config.cache_size);

   _free_bytes_eviction_threshold = scsvmoc_config.cache_size * .1;
