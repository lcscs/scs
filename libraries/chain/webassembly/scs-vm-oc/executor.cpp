#include <lcscs/chain/webassembly/scs-vm-oc/executor.hpp>
#include <lcscs/chain/webassembly/scs-vm-oc/code_cache.hpp>
#include <lcscs/chain/webassembly/scs-vm-oc/memory.hpp>
#include <lcscs/chain/webassembly/scs-vm-oc/intrinsic_mapping.hpp>
...
#error scs VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_code_start+cb_in_main_segment->execution_thread_code_length)
         siglongjmp(*cb_in_main_segment->jmp, scsVMOC_EXIT_CHECKTIME_FAIL);
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, scsVMOC_EXIT_SEGV);
...
static intrinsic grow_memory_intrinsic scsVMOC_INTRINSIC_INIT_PRIORITY("scsvmoc_internal.grow_memory", IR::FunctionType::get(IR::ResultType::i32,{IR::ValueType::i32,IR::ValueType::i32}),
  (void*)&scs_vm_oc_grow_memory,
  boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("scsvmoc_internal.grow_memory"))).value()
);

//This is effectively overriding the lcscs_exit intrinsic in wasm_interface
static void lcscs_exit(int32_t code) {
   siglongjmp(*scs_vm_oc_get_jmp_buf(), scsVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
static intrinsic lcscs_exit_intrinsic("env.lcscs_exit", IR::FunctionType::get(IR::ResultType::none,{IR::ValueType::i32}), (void*)&lcscs_exit,
  boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("env.lcscs_exit"))).value()
);

static void throw_internal_exception(const char* const s) {
   *reinterpret_cast<std::exception_ptr*>(scs_vm_oc_get_exception_ptr()) = std::make_exception_ptr(wasm_execution_error(FC_LOG_MESSAGE(error, s)));
   siglongjmp(*scs_vm_oc_get_jmp_buf(), scsVMOC_EXIT_EXCEPTION);
   __builtin_unreachable();
}

#define DEFINE_scsVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function scsVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
     boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING(#module "." #name))).value() \
   ); \
	void name()

DEFINE_scsVMOC_TRAP_INTRINSIC(scsvmoc_internal,depth_assert) {
   throw_internal_exception("Exceeded call depth maximum");
}

DEFINE_scsVMOC_TRAP_INTRINSIC(scsvmoc_internal,div0_or_overflow) {
   throw_internal_exception("Division by 0 or integer overflow trapped");
}

DEFINE_scsVMOC_TRAP_INTRINSIC(scsvmoc_internal,indirect_call_mismatch) {
   throw_internal_exception("Indirect call function type mismatch");
}

DEFINE_scsVMOC_TRAP_INTRINSIC(scsvmoc_internal,indirect_call_oob) {
   throw_internal_exception("Indirect call index out of bounds");
}

DEFINE_scsVMOC_TRAP_INTRINSIC(scsvmoc_internal,unreachable) {
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. scs VM OC may not run correctly on this platform");
...
void executor::execute(const code_descriptor& code, const memory& mem, apply_context& context) {
   ...
   cb->current_call_depth_remaining = lcscs::chain::wasm_constraints::maximum_call_depth+2;
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      case 0:
         ...
         break;
      //case 1: clean lcscs_exit
      case scsVMOC_EXIT_CHECKTIME_FAIL:
         context.trx_context.checktime();
         break;
      case scsVMOC_EXIT_SEGV:
         scs_ASSERT(false, wasm_execution_error, "access violation");
         break;
      case scsVMOC_EXIT_EXCEPTION: //exception
