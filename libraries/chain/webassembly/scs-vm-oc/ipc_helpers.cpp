...
bool write_message_with_fds(boost::asio::local::datagram_protocol::socket& s, const scsvmoc_message& message, const std::vector<wrapped_fd>& fds) {
   return write_message_with_fds(s.native_handle(), message, fds);
}

bool write_message_with_fds(int fd_to_send_to, const scsvmoc_message& message, const std::vector<wrapped_fd>& fds) {
   struct msghdr msg = {};
   struct cmsghdr* cmsg;
