...
#define REQUIRE_MATCHING_OBJECT(left, right) { auto a = fc::variant( left ); auto b = fc::variant( right ); BOOST_REQUIRE_EQUAL( true, a.is_object() ); \
   BOOST_REQUIRE_EQUAL( true, b.is_object() ); \
   auto filtered = ::lcscs::testing::filter_fields( a.get_object(), b.get_object() ); \
...
lcscs::chain::asset core_from_string(const std::string& s);
...
namespace lcscs { namespace testing {
   ...
   class base_tester {
      public:
         ...
         static std::pair<controller::config, genesis_state> default_config(const fc::temp_directory& tempdir, optional<uint32_t> genesis_max_inline_action_size = optional<uint32_t>{}, optional<uint32_t> config_max_nonprivileged_inline_action_size = optional<uint32_t>{}) {
            ...
            cfg.scsvmoc_config.cache_size = 1024*1024*8;
            ...
            for(int i = 0; i < boost::unit_test::framework::master_test_suite().argc; ++i) {
               ...
               else if(boost::unit_test::framework::master_test_suite().argv[i] == std::string("--scs-vm"))
                  cfg.wasm_runtime = chain::wasm_interface::vm_type::scs_vm;
               else if(boost::unit_test::framework::master_test_suite().argv[i] == std::string("--scs-vm-jit"))
                  cfg.wasm_runtime = chain::wasm_interface::vm_type::scs_vm_jit;
               else if(boost::unit_test::framework::master_test_suite().argv[i] == std::string("--scs-vm-oc"))
                  cfg.wasm_runtime = chain::wasm_interface::vm_type::scs_vm_oc;
            }
  ...
  /**
   * Utility predicate to check whether an lcscs_assert message is equivalent to a given string
   */
  struct lcscs_assert_message_is {
     lcscs_assert_message_is( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }

     bool operator()( const lcscs_assert_message_exception& ex );

     string expected;
  };

  /**
   * Utility predicate to check whether an lcscs_assert message starts with a given string
   */
  struct lcscs_assert_message_starts_with {
     lcscs_assert_message_starts_with( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }
  ...
  /**
   * Utility predicate to check whether an lcscs_assert_code error code is equivalent to a given number
   */
  struct lcscs_assert_code_is {
     lcscs_assert_code_is( uint64_t error_code )
           : expected( "assertion failure with error code: " ) {
        expected.append( std::to_string(error_code) );
     }

     bool operator()( const lcscs_assert_code_exception& ex );
